...
function CloudManager._applyMultiDomain(saveObj)
    ...
    if domains.skills then
        ...
        if d.playerEquippedSkills then
            playerEquippedSkills = d.playerEquippedSkills
            -- 迁移: 旧格式(全局数组) → 新格式(按武将分配)
            if playerEquippedSkills and #playerEquippedSkills > 0 and type(playerEquippedSkills[1]) == "number" then
                local oldSkills = {}
                for _, si in ipairs(playerEquippedSkills) do oldSkills[#oldSkills + 1] = si end
                local defaultHero = nil
                if rawget(_G, "playerHeroes") then
                    for hIdx, hero in pairs(playerHeroes) do
                        if hero.owned then
                            if not defaultHero or hIdx < defaultHero then defaultHero = hIdx end
                        end
                    end
                end
                defaultHero = defaultHero or 1
                playerEquippedSkills = {}
                playerEquippedSkills[defaultHero] = oldSkills
                print("[云存档] playerEquippedSkills迁移: 全局数组→按武将, 默认分配给武将#" .. tostring(defaultHero))
            end
        end
...
--- 管理员: 获取当前封禁名单 (服务端权威 - 通过 Server.lua 中转)
---@param callback fun(bans: table|nil, err: string|nil)
function CloudManager.AdminGetBanList(callback)
    local ClientNet = rawget(_G, "_ClientNet")
    if not ClientNet then
        if callback then callback(nil, "网络未连接") end
        return
    end
    ClientNet.CallAdmin("get_ban_list", {}, {
        ok = function(payload)
            if callback then callback(payload.bans or {}, nil) end
        end,
        error = function(errMsg)
            if callback then callback(nil, tostring(errMsg)) end
        end,
    })
end

--- 管理员: 发布封禁名单 (服务端权威 - 覆盖式写入)
...
function CloudManager.AdminPublishBanList(bans, callback)
    local ClientNet = rawget(_G, "_ClientNet")
    if not ClientNet then
        if callback then callback(false, "网络未连接") end
        return
    end
    ClientNet.CallAdmin("publish_ban_list", { bans = bans }, {
        ok = function()
            print("[管理员] 封禁名单已发布(服务端权威), 条目数: " .. CloudManager._tableCount(bans))
            if callback then callback(true, nil) end
        end,
        error = function(errMsg)
            if callback then callback(false, tostring(errMsg)) end
        end,
    })
end
...
function CloudManager.AdminHidePlayerRank(targetUid, callback)
    ...
    ClientNet.CallAdmin("hide_rank", { targetUid = targetUid }, {
        ok = function()
            if callback then callback(true, "已隐藏") end
        end,
        error = function(errMsg)
            if callback then callback(false, tostring(errMsg)) end
        end,
...
function CloudManager.AdminUnhidePlayerRank(targetUid, callback)
    ...
    ClientNet.CallAdmin("unhide_rank", { targetUid = targetUid }, {
        ok = function()
            if callback then callback(true, "已恢复显示") end
        end,
        error = function(errMsg)
            if callback then callback(false, tostring(errMsg)) end
        end,
...
function CloudManager.AdminPermanentBan(targetUid, callback)
    ...
    ClientNet.CallAdmin("permanent_ban", { targetUid = targetUid }, {
        ok = function()
            if callback then callback(true, "已永久封禁") end
        end,
        error = function(errMsg)
            if callback then callback(false, tostring(errMsg)) end
        end,
...
function CloudManager.AdminFullUnban(targetUid, callback)
    ...
    ClientNet.CallAdmin("full_unban", { targetUid = targetUid }, {
        ok = function()
            if callback then callback(true, "已完全解禁") end
        end,
        error = function(errMsg)
            if callback then callback(false, tostring(errMsg)) end
        end,
...
function CloudManager.SendMail(targetUid, subject, body, rewards, callback)
    ...
    if rewards and #rewards > 0 then
        ...
        ClientNet.CallAdmin("send_mail", {
            ...
            ok = function()
                print("[邮件] 管理员邮件发送成功(服务端权威) → " .. tostring(targetUid))
                if callback then callback(true, "发送成功") end
            end,
            error = function(errMsg)
                print("[邮件] 管理员邮件发送失败: " .. tostring(errMsg))
                if callback then callback(false, tostring(errMsg)) end
            end,
        })
        return
...
function CloudManager.BroadcastMail(subject, body, rewards, callback)
    ...
    ClientNet.CallAdmin("broadcast_mail", {
        subject = subject,
        body = body,
        rewards = rewards or {},
    }, {
        ok = function()
            print("[邮件] 广播邮件已发布(服务端权威): " .. tostring(subject))
            if callback then callback(true, "广播成功") end
        end,
        error = function(errMsg)
            print("[邮件] 广播邮件失败: " .. tostring(errMsg))
            if callback then callback(false, tostring(errMsg)) end
        end,
    })
end
...
--- 判断当前玩家是否为管理员（由服务端 Welcome 下发，客户端不保存 UID 列表）
---@return boolean
function CloudManager.IsAdmin()
    local ns = rawget(_G, "netState")
    return ns ~= nil and ns.isAdmin == true
end
