...
function HandleServerReady(eventType, eventData)
    print("[Main] ServerReady 事件触发 - 初始化网络层")
    local ClientNet = require("network.Client")
    ClientNet.Start()

    -- 服务端权威模式：STATE_SYNC 到达后重新加载游戏数据
    -- Start() 中的 LoadGameProgress 用本地数据先渲染，避免白屏
    -- 这里注册回调，在服务端数据到达后用权威数据覆盖
    rawset(_G, "OnFullStateSync", function(newState)
        print("[Main] 收到服务端全量数据，重新加载游戏进度")

        -- 用服务端权威数据重新加载（会调用 _applyMultiDomain）
        LoadGameProgress()
