...
function Server.Start()
    ...
    -- 管理员事件（服务端权威）
    SubscribeToEvent(EVENTS.ADMIN_REQUEST, "HandleAdminRequest")
...
function HandleAdminRequest(eventType, eventData)
    local connection = eventData["Connection"]:GetPtr("Connection")
    local connKey = tostring(connection)
    local userId = connectionUserIds_[connKey]
    if not userId then return end
    ...
    CloudService.HandleAdminRequest(userId, connection, requestId, action, params)
end
