...
function Client.Start()
    ...
    if not eventsSubscribed_ then
        ...
        SubscribeToEvent(EVENTS.ADMIN_RESPONSE, "HandleAdminResponse")
...
local nextAdminRequestId_ = 1
local pendingAdminRequests_ = {}

function HandleAdminResponse(eventType, eventData)
    local requestId = eventData["RequestId"]:GetInt()
    local success = eventData["Success"]:GetBool()
    local payloadStr = eventData["Payload"]:GetString()
    local errMsg = eventData["Error"]:GetString()

    local request = pendingAdminRequests_[requestId]
    pendingAdminRequests_[requestId] = nil
    ...
    if success then
        ...
        if request.error then request.error(errMsg or "admin request error") end
    end
end

--- 发送管理员请求到服务端（服务端会校验 UID）
function Client.CallAdmin(action, params, callbacks)
    callbacks = callbacks or {}

    if not ensureServerConnection() then
        if callbacks.error then callbacks.error("no server connection") end
        return false
    end

    if not (netState.connected and netState.serverReady) then
        if callbacks.error then callbacks.error("server not ready") end
        return false
    end

    local requestId = nextAdminRequestId_
    nextAdminRequestId_ = nextAdminRequestId_ + 1
    pendingAdminRequests_[requestId] = callbacks

    local data = VariantMap()
    data["RequestId"] = Variant(requestId)
    data["Action"] = Variant(action)
    data["Params"] = Variant(cjson.encode(params or {}))
    serverConnection_:SendRemoteEvent(EVENTS.ADMIN_REQUEST, true, data)
    return true
end
