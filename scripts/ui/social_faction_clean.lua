...
local function FactionRoleName(role)
    ...
    local map = { leader = "Leader", vice_leader = "Vice Leader", strategist = "Strategist", vanguard = "Vanguard", diplomat = "Diplomat", elite = "Elite", member = "Member" }
    return map[role] or "Member"
end
...
local function DrawFactionRankOverlay(W, H)
    ...
    DrawWhiteInkText(cx, py + 30, "Faction Rank")
    menuBtnRects.factionRankClose = { x = cx - 45, y = py + ph - 46, w = 90, h = 36 }
    FactionButton(cx - 45, py + ph - 46, 90, 36, "Close", false)
    local rows = factionUI.rankList or {}
    if factionUI.rankLoading then DrawWhiteInkText(cx, py + ph / 2, "Loading..."); return end
    if #rows == 0 then DrawWhiteInkText(cx, py + ph / 2, "No rank data"); return end
    for i, entry in ipairs(rows) do
        ...
        nvgText(vg, px + 70, y + 18, FactionSafeText(entry.name, "Faction " .. i), nil); nvgTextAlign(vg, NVG_ALIGN_RIGHT + NVG_ALIGN_MIDDLE)
...
function DrawFactionSubView(W, H, bodyTop, pad, cx, info)
    ...
    FactionButton(pad, bodyTop + 4, 80, 32, "< Back", false)
    ...
    if factionUI.subView == "upgrade" then
        ...
        DrawWhiteInkText(cx, top + 24, "Faction Upgrade")
        DrawWhiteInkText(cx, top + 66, "Lv." .. tostring(lvInfo.level or 1))
        nvgText(vg, cx, top + 106, "Power Bonus +" .. tostring(lvInfo.buffPercent or 0) .. "%", nil)
        nvgText(vg, cx, top + 138, tostring(lvInfo.exp or 0) .. " / " .. tostring(lvInfo.nextLevelExp or 0), nil)
        nvgText(vg, cx, top + 166, "Donate to gain experience", nil)
        menuBtnRects.factionRankBtn = { x = cx - 85, y = top + 180, w = 170, h = 34 }
        FactionButton(cx - 85, top + 180, 170, 34, "Open Rank", false)
        return
    end
    if factionUI.subView == "donate" then
        ...
        nvgText(vg, pad + 18, top + 18, "Jade: " .. tostring(playerInfo.jade or 0), nil)
        nvgText(vg, pad + 18, top + 46, "My Contrib: " .. tostring(CloudManager.GetMyContribution and CloudManager.GetMyContribution() or 0), nil)
        nvgText(vg, pad + 18, top + 74, "Faction Fund: " .. tostring(CloudManager.GetFactionFunds and CloudManager.GetFactionFunds() or 0), nil)
        nvgText(vg, cx, top + 116, "Choose amount", nil)
        ...
        FactionButton(cx - 90, top + 188, 180, 42, factionUI.donating and "Working..." or ("Donate " .. tostring(factionUI.donateAmount or 0)), not factionUI.donating)
        return
    end
    if factionUI.subView == "announce" then
        ...
        nvgText(vg, pad + 14, top + 12, "Announcement:", nil)
        nvgText(vg, pad + 14, top + 38, FactionSafeText(CloudManager.GetFactionAnnouncement and CloudManager.GetFactionAnnouncement() or "", "No announcement"), nil)
        menuBtnRects.factionAnnounceInput = { x = pad + 12, y = top + 104, w = panelW - 24, h = 40 }
        FactionPanel(pad + 12, top + 104, panelW - 24, 40)
        nvgText(vg, pad + 22, top + 126, factionUI.announceInput ~= "" and factionUI.announceInput or "Tap to input", nil)
        menuBtnRects.factionAnnounceSave = { x = cx - 60, y = top + 152, w = 120, h = 38 }
        FactionButton(cx - 60, top + 152, 120, 38, "Save", true)
        return
    end
    if factionUI.subView == "contrib" then
        ...
        if factionUI.contribLoading then DrawWhiteInkText(cx, top + 80, "Loading..."); return end
        if #(factionUI.contribList or {}) == 0 then DrawWhiteInkText(cx, top + 80, "No contrib data"); return end
        for i, entry in ipairs(factionUI.contribList or {}) do
            ...
            nvgText(vg, pad + 60, y + 18, FactionSafeText(entry.name, "Member " .. tostring(entry.uid or i)), nil)
            nvgText(vg, pad + panelW - 16, y + 18, tostring(entry.amount or 0), nil)
        end
        return
    end
    DrawWhiteInkText(cx, top + 40, "Feature under cleanup")
end
...
function DrawFactionScreen()
    ...
    menuBtnRects.factionBack = { x = 10, y = 10, w = 100, h = 44 }; FactionButton(10, 10, 100, 44, "< Back", false)
    DrawWhiteInkText(cx, 32, "Faction")
    ...
    if factionUI.renamePopup then
        ...
        DrawWhiteInkText(cx, py + 30, "Rename Faction")
        nvgText(vg, cx, py + 56, "Cost: 1000 Jade", nil)
        ...
        FactionButton(menuBtnRects.factionRenameYes.x, menuBtnRects.factionRenameYes.y, 110, 38, "Confirm", true)
        FactionButton(menuBtnRects.factionRenameNo.x, menuBtnRects.factionRenameNo.y, 110, 38, "Cancel", false)
    end
    if factionUI.confirmPopup then
        local pw, ph = W - 180, 160
        local px, py = cx - pw / 2, H / 2 - ph / 2
        nvgBeginPath(vg); nvgRect(vg, 0, 0, W, H); nvgFillColor(vg, nvgRGBA(0, 0, 0, 160)); nvgFill(vg); FactionPanel(px, py, pw, ph)
        DrawWhiteInkText(cx, py + 46, FactionSafeText(factionUI.confirmPopup.msg, "Confirm this action?"))
        menuBtnRects.factionPopupYes = { x = cx - 120, y = py + ph - 52, w = 110, h = 38 }
        menuBtnRects.factionPopupNo = { x = cx + 10, y = py + ph - 52, w = 110, h = 38 }
        FactionButton(menuBtnRects.factionPopupYes.x, menuBtnRects.factionPopupYes.y, 110, 38, "Confirm", true)
        FactionButton(menuBtnRects.factionPopupNo.x, menuBtnRects.factionPopupNo.y, 110, 38, "Cancel", false)
    end
    ...
    local contentTop = 64
    if hasFaction then
        local tabs = { { id = "info", label = "Info" }, { id = "members", label = "Members" }, { id = "chat", label = "Chat" } }
        local myLevel = FactionRoleLevel(info.role)
        if myLevel >= 5 then table.insert(tabs, { id = "apply", label = "Apply" }) end
        ...
        elseif factionUI.tab == "info" then
            ...
            DrawWhiteInkText(cx, bodyTop + 22, FactionSafeText(info.name, "Unknown Faction"))
            nvgText(vg, pad + 14, bodyTop + 56, "Role: " .. FactionRoleName(info.role), nil)
            nvgText(vg, pad + 14, bodyTop + 84, "Leader: " .. FactionSafeText(factionUI.leaderNickname or info.leaderName, "Unknown"), nil)
            nvgText(vg, pad + 14, bodyTop + 112, "Fund: " .. tostring(CloudManager.GetFactionFunds and CloudManager.GetFactionFunds() or 0), nil)
            nvgText(vg, pad + 14, bodyTop + 140, "Announce: " .. FactionSafeText(CloudManager.GetFactionAnnouncement and CloudManager.GetFactionAnnouncement() or "", "None"), nil)
            menuBtnRects.factionLeave = { x = cx - 70, y = bodyTop + 190, w = 140, h = 38 }
            FactionButton(cx - 70, bodyTop + 190, 140, 38, info.role == "leader" and "Dismiss" or "Leave", false)
            if myLevel >= 5 then menuBtnRects.factionRename = { x = pad, y = bodyTop + 190, w = 140, h = 38 }; FactionButton(pad, bodyTop + 190, 140, 38, "Rename", false) end
            local feats = {
                { id = "manage", label = "Manage" }, { id = "chat", label = "Chat" }, { id = "upgrade", label = "Upgrade" }, { id = "donate", label = "Donate" },
                { id = "signIn", label = (CloudManager.HasSignedInToday and CloudManager.HasSignedInToday()) and "Signed" or "Sign In" },
                { id = "announce", label = "Announce" }, { id = "rank", label = "Rank" }, { id = "contrib", label = "Contrib" }
            }
        ...
        elseif factionUI.tab == "members" then
            ...
            if factionUI.loading then DrawWhiteInkText(cx, bodyTop + 60, "Loading...")
            elseif #(factionUI.members or {}) == 0 then DrawWhiteInkText(cx, bodyTop + 60, "No members")
            else
                for i, mem in ipairs(factionUI.members or {}) do
                    ...
                    nvgText(vg, pad + 14, y + 20, FactionSafeText(mem.nickname, "Player " .. tostring(mem.userId or i)), nil)
        ...
        elseif factionUI.tab == "apply" then
            ...
            if factionUI.applyLoading then DrawWhiteInkText(cx, bodyTop + 60, "Loading...")
            elseif #(factionUI.applications or {}) == 0 then DrawWhiteInkText(cx, bodyTop + 60, "No applications")
            else
                for i, app in ipairs(factionUI.applications or {}) do
                    local y = bodyTop + (i - 1) * 62
                    if y + 56 > H - 10 then break end
                    FactionPanel(pad, y, W - pad * 2, 56)
                    nvgText(vg, pad + 14, y + 28, FactionSafeText(app.nickname, "Player " .. tostring(app.userId or i)), nil)
                    ...
                    FactionButton(W - pad - 122, y + 13, 56, 30, "Yes", true)
                    FactionButton(W - pad - 56, y + 13, 56, 30, "No", false)
                end
            end
        else
            ...
            if #msgs == 0 then DrawWhiteInkText(cx, bodyTop + chatAreaH / 2, "No chat yet")
            else
                local startIdx = math.max(1, #msgs - 7)
                for i = startIdx, #msgs do
                    local msg, y = msgs[i], bodyTop + 16 + (i - startIdx) * 28
                    nvgText(vg, pad + 12, y, FactionSafeText(msg.senderName or msg.name, "Member") .. ": " .. FactionSafeText(msg.text, ""), nil)
                end
            end
            ...
            nvgText(vg, pad + 10, H - 29, factionUI.chatInput ~= "" and factionUI.chatInput or "Tap to input", nil)
            FactionButton(W - pad - 80, H - 46, 80, 34, "Send", true)
        end
    ...
    FactionButton(pad + 2, contentTop, tabW - 4, 38, "List", factionUI.tab == "list")
    FactionButton(pad + tabW + 2, contentTop, tabW - 4, 38, "Create", factionUI.tab == "create")
    local bodyTop = contentTop + 48
    if factionUI.tab == "list" then
        ...
        if factionUI.applyStatus == "pending" then
            ...
            FactionButton(cx - 55, bodyTop, 110, 36, "Refresh", false)
            nvgText(vg, cx, bodyTop + 54, "Application pending...", nil)
            bodyTop = bodyTop + 84
        end
        if factionUI.loading then DrawWhiteInkText(cx, bodyTop + 80, "Loading factions...")
        elseif #(factionUI.factions or {}) == 0 then DrawWhiteInkText(cx, bodyTop + 80, "No factions yet")
        else
            for i, fac in ipairs(factionUI.factions or {}) do
                ...
                nvgText(vg, pad + 12, y + 10, FactionSafeText(fac.name, "Faction " .. i), nil)
                nvgText(vg, pad + 12, y + 40, "Lv." .. tostring(fac.level or 1), nil)
                nvgText(vg, pad + 120, y + 40, "Members " .. tostring(fac.memberCount or 0), nil)
                menuBtnRects["factionApply_" .. i] = { x = W - pad - 104, y = y + 22, w = 92, h = 34, campId = fac.id, campName = fac.name }
                FactionButton(W - pad - 104, y + 22, 92, 34, "Apply", true)
            end
        end
    else
        FactionPanel(pad, bodyTop, W - pad * 2, 200)
        DrawWhiteInkText(cx, bodyTop + 26, "Create Faction")
        nvgText(vg, cx, bodyTop + 52, "Cost: 5000 Jade", nil)
        ...
        nvgText(vg, pad + 22, bodyTop + 93, factionUI.createName ~= "" and factionUI.createName or "Tap faction name", nil)
        nvgText(vg, pad + 22, bodyTop + 145, factionUI.createDesc ~= "" and factionUI.createDesc or "Tap faction desc", nil)
        menuBtnRects.factionCreate = { x = cx - 75, y = bodyTop + 184, w = 150, h = 40 }
        FactionButton(cx - 75, bodyTop + 184, 150, 40, "Create", true)
    end
end
