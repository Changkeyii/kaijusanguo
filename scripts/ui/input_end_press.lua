...
function HandleSkillCodexEndPress(sx, sy)
    ...
    local dx, dy = ScreenToDesign(sx, sy)
    local dragDistX = math.abs(dx - (skillCodexState.dragStartX or dx))
    local dragDistY = math.abs(dy - (skillCodexState.dragStartY or dy))
    local dragDist = math.max(dragDistX, dragDistY)
    if dragDist < 10 then
        if phaseChangeCooldown <= 0 and not skillPopup.show then
    ...
    skillCodexState.dragStartX = nil
    skillCodexState.dragStartY = nil
    skillCodexState.dragLastX = nil
...
function HandleSidebarButtonClick(dx, dy)
    ...
    elseif HitR(menuBtnRects.skillCodex) then
        ...
        PushPhase("SKILL_CODEX"); skillCodexState.scrollX = 0; skillCodexState.scrollVelX = 0; skillCodexState.scrollY = 0; skillCodexState.scrollVel = 0
...
function HandleEndPressDragRelease(sx, sy, touchId)
    ...
    -- 奖励弹窗滚动释放
    if gameState.rewardDragging then
        gameState.rewardDragging = false
        return true
    end
    ...
    if settingsPage.isOpen and gameState.phase == "MENU" then
        ...
        settingsPage.draggingSpriteLimit = false
        return true
    end
        ...
        if dragDist < 10 then
            ...
            for _, rect in pairs(equipPieceRects) do
                ...
                if HitBtn(rect.equipBtnRect) then
                    local itemObj = FindOwnedByUid(info.uid)
                    if itemObj then
                        EquipItemToHero(equipScreenState.selectedHero, slotIdx, itemObj.uid)
                ...
                if HitBtn(rect.unequipRect) then
                    UnequipFromHero(equipScreenState.selectedHero, slotIdx)
...
function EndPress(sx, sy, touchId)
    ...
    if gameState.phase == "GM_PANEL" then
        ...
        if GMPanel.IsActive() then
            if GMPanel.GetTab() == "editor" then
                ...
                if TDEditor.IsActive() then TDEditor.handleEndPress(sx, sy) end
            end
        end
        return
    end
    ...
    -- 战斗地图拖拽平移结束
    if _battlePanning then
        _battlePanning = false
        _battlePanTouchId = nil
        return
    end

    -- 塔防编辑器松手 (已合并到 GM_PANEL, 在顶部处理)
    -- 塔防选择界面松手
    if gameState.phase == "TD_SELECT" and tdSelectState then
        local TDSelect = require("systems.td.td_select")
        TDSelect.handleEndPress(sx, sy, touchId)
        return
    end
    -- 塔防战斗松手
    if gameState.phase == "TD_BATTLE" and tdState then
        ...
        TDInput.handleEndPress(sx, sy, touchId)
        return
    end
