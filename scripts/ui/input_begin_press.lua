...
function BeginPress(sx, sy, touchId)
    ...
    -- ======== 统一规则弹窗交互 (全局最高优先级) ========
        ...
        if menuBtnRects.ranked and HitRect(menuBtnRects.ranked) then
            -- 排位入口（暂未开放）
            PlaySFX(AUDIO.sfx_click)
            ShowToast("排位赛即将开放，敬请期待")
    ...
    if gameState.phase == "SKILL_DETAIL" then
        ...
        elseif skillDetailEquipBtnRect and HitRect(skillDetailEquipBtnRect) then
            ...
            if SKILL_DEFS[curIdx] and SKILL_DEFS[curIdx].notAvailable then
                AddFloatText(DESIGN_W / 2, DESIGN_H * 0.4, "此武技暂未开放", 1.0, { 160, 150, 130 }, 18)
    ...
    if gameState.battlePhase == "FIGHT" and gameState.selectedTroopType then
        ...
        local inBattleZone = dx >= BATTLE_ZONE.left and dx <= BATTLE_ZONE.right
            and dy >= BATTLE_ZONE.top and dy <= BATTLE_ZONE.bottom
        if inBattleZone then
            ...
            if nearEnemy then
                ...
                AddFloatText(dx, dy - 20, tt.name .. " 进攻!", 1.0, { 255, 100, 60 }, 18)
            else
                -- 移动指令: 朝点击位置集结
                gameState.troopOrders[tk] = {
                    type = "move",
                ...
                local tt = TROOP_TYPES[tk]
                AddFloatText(dx, dy - 20, tt.name .. " 集结", 1.0, { 60, 200, 255 }, 18)
            end
                ...
                if sdx * sdx + sdy * sdy <= sb.r * sb.r then
                    ...
                    if skill then
                        if not skill.unlocked then
                            AddFloatText(DESIGN_W / 2, DESIGN_H * 0.45, "武技未解锁", 0.8, { 200, 160, 100 }, 18)
                        elseif skill.cooldown > 0 then
                            local cdLeft = math.ceil(skill.cooldown)
                            AddFloatText(DESIGN_W / 2, DESIGN_H * 0.45, "冷却中 " .. cdLeft .. "s", 0.8, { 200, 160, 100 }, 18)
                        else
                            -- 直接开始拖拽瞄准
                            ...
                            AddFloatText(DESIGN_W / 2, DESIGN_H * 0.45, skill.name .. " - 拖拽瞄准", 0.8, skill.color, 18)
                        end
                    end
                    PlaySFX(AUDIO.sfx_click)
                    return
