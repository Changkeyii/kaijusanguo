...
function DrawMenuScreen()
    ...
    -- ===========================
    local centerAreaTop = H * 0.007
    local bottomBarH = H * 0.140
    local bottomBarY = H - bottomBarH - H * 0.007
    local centerAreaBottom = bottomBarY - H * 0.010
    ...
    -- ===========================
    local sideBtnW = W * 0.086
    local sideBtnH = H * 0.119
    local sideGap = H * 0.009
    local sideX = W * 0.004
    local leftEndY = bottomBarY - H * 0.007       -- 闁圭厧鐡ㄥú鐔煎磿閹绢喗鍋╂繛鍡楃箰濮ｅ姊婚崒婊庢缂?
    local leftStartY = H * 0.193
    local leftViewH = leftEndY - leftStartY

    -- 閻庡綊娼荤紓姘跺疾閸洖绠板鑸靛姈鐏忥箓姊洪弶璺ㄐら柣?(闁汇埄鍨界粻鎴澝瑰鈧?"
    local leftButtons = {
        -- { label = "势力",      key = "faction",    colors = {70, 55, 30},  mod = nil,                   icon = IMG.menuIcons and IMG.menuIcons[8] }, -- 暂时隐藏
        -- { label = "好友",      key = "friends",    colors = {40, 65, 70},  mod = nil,                   icon = IMG.menuIcons and IMG.menuIcons[9] }, -- 暂时隐藏
        ...
        { label = "武将图鉴",    key = "codex",      colors = {60, 45, 80},  mod = moduleState.heroes,    icon = IMG.menuIcons and IMG.menuIcons[1] },
    ...
    -- 好友/阵营红点检查暂时隐藏
    if false then local now = os.time()
    ...
    local bottomButtons = {
        ...
        { label = "召唤",  key = "gachaSeal",  colors = {80, 50, 130}, primary = false, mod = nil,                icon = IMG.sealItem1 },
        { label = "玩法",  key = "campaign",   colors = {160, 80, 30}, primary = false, mod = nil,                icon = IMG.iconGameplay },
    }
    local botBtnCount = #bottomButtons
    local botPad = W * 0.078  -- 閻庡綊娼荤紓姘跺疾閸洘鍋╂繛鍡楃箰濮ｅ銆掑鈧崟顓炵効缂備礁鏈钘壩?
    local botTotalW = W - botPad - W * 0.010
    local botBtnGap = W * 0.006
    local botBtnW = (botTotalW - (botBtnCount - 1) * botBtnGap) / botBtnCount
    local botBtnH = H * 0.126
    local botBtnY = bottomBarY + (bottomBarH - botBtnH) / 2

    local pulse = 0.7 + 0.3 * math.sin(t * 2.5)
    for i, bb in ipairs(bottomButtons) do
        local bx = botPad + (i - 1) * (botBtnW + botBtnGap)
        local by = botBtnY
        ...
        if bb.key == "quickBattle" then
            menuBtnRects.quickBattle = rect
        ...
        elseif bb.key == "campaign" then
            menuBtnRects.campaign = rect
        end
    end
    ...
    if false then -- 聊天功能暂时隐藏
        ...
        if not worldChatUI.expanded then
            ...
            local miniW = math.min(W * 0.6, W * 0.332)
            local miniH = H * 0.056
            local miniX = (W - miniW) / 2
            local miniY = bottomBarY - miniH - H * 0.010
    ...
    -- ===========================
    local panelW = W * 0.225
    local panelH = H * 0.147
    local panelX = W * 0.006
    local panelY = H * 0.007
    ...
    local avatarSize = H * 0.088
    local avatarX = panelX + W * 0.008
    ...
    -- ===========================
    local jadeBoxW = W * 0.176
    local jadeBoxH = H * 0.060
    local jadeBoxX = W - jadeBoxW - W * 0.010
    local jadeBoxY = H * 0.007
    ...
    -- 濡ょ姷鍋涢悘婵嬪箟?(+)
    local adBtnW = W * 0.029
    local adBtnH = H * 0.039
    ...
    if not allModulesReady then
        local dlBtnW = W * 0.070
        local dlBtnH = H * 0.042
        local dlBtnX = W - dlBtnW - W * 0.010
        local dlBtnY = jadeBoxY + jadeBoxH + H * 0.032
    ...
    if gameState.campaignModePopup then
        ...
        local cpw, cph = 300, 200
        local cpx, cpy = (W - cpw) / 2, (H - cph) / 2
        nvgBeginPath(vg); nvgRoundedRect(vg, cpx, cpy, cpw, cph, 12)
        ...
        nvgFontFaceId(vg, GetMainFont())
        nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_TOP)
        nvgFontSize(vg, 22); nvgFillColor(vg, nvgRGBA(255, 220, 120, 255))
        nvgText(vg, W / 2, cpy + 16, "玩法模式", nil)

        -- 两个选项按钮
        local optW, optH = 220, 44
        local optX = (W - optW) / 2
        local opt1Y = cpy + 60
        local opt2Y = cpy + 116

        -- 放置守城
        nvgBeginPath(vg); nvgRoundedRect(vg, optX, opt1Y, optW, optH, 8)
        nvgFillColor(vg, nvgRGBA(140, 70, 25, 200)); nvgFill(vg)
        nvgStrokeColor(vg, nvgRGBA(255, 180, 80, 180)); nvgStrokeWidth(vg, 1); nvgStroke(vg)
        nvgFontSize(vg, 18); nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
        nvgFillColor(vg, nvgRGBA(255, 240, 200, 240))
        nvgText(vg, W / 2, opt1Y + optH / 2, "放置守城", nil)
        menuBtnRects.campaignTD = { x = optX, y = opt1Y, w = optW, h = optH }

        -- 战场（暂未开放）
        nvgBeginPath(vg); nvgRoundedRect(vg, optX, opt2Y, optW, optH, 8)
        nvgFillColor(vg, nvgRGBA(60, 55, 50, 200)); nvgFill(vg)
        nvgStrokeColor(vg, nvgRGBA(120, 110, 100, 120)); nvgStrokeWidth(vg, 1); nvgStroke(vg)
        nvgFontSize(vg, 18)
        nvgFillColor(vg, nvgRGBA(160, 150, 140, 180))
        nvgText(vg, W / 2, opt2Y + optH / 2, "战场（暂未开放）", nil)
        menuBtnRects.campaignBattle = { x = optX, y = opt2Y, w = optW, h = optH }

        -- 关闭按钮
        local closeW, closeH = 80, 30
        local closeX = (W - closeW) / 2
        local closeY = cpy + cph - 38
        nvgBeginPath(vg); nvgRoundedRect(vg, closeX, closeY, closeW, closeH, 6)
        nvgFillColor(vg, nvgRGBA(50, 45, 35, 200)); nvgFill(vg)
        nvgStrokeColor(vg, nvgRGBA(150, 130, 100, 150)); nvgStrokeWidth(vg, 1); nvgStroke(vg)
        nvgFontSize(vg, 16); nvgFillColor(vg, nvgRGBA(220, 210, 190, 220))
        nvgText(vg, W / 2, closeY + closeH / 2, "关闭", nil)
        menuBtnRects.campaignClose = { x = closeX, y = closeY, w = closeW, h = closeH }
    end
end
...
function DrawBtnAdjustMode()
    ...
    local rbBtnW = W * 0.082
    local rbBtnH = H * 0.063
    local rbGap = H * 0.010
    local rbRightMargin = W * 0.004
    local rbStartY = H * 0.049 + rbOfsY
