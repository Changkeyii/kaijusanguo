...
function DrawFriendsScreen()
    ...
    menuBtnRects.friendsBack = { x = 10, y = 10, w = 100, h = 44 }; FriendBtn(10, 10, 100, 44, "< 返回", false)
    DrawWhiteInkText(cx, 32, "好友")
    local tabs = { { id = "list", label = "列表" }, { id = "add", label = "添加" }, { id = "requests", label = "请求" } }
    ...
    if friendsUI.tab == "list" then
        ...
        if friendsUI.loading then DrawWhiteInkText(cx, bodyTop + 80, "加载中...")
        elseif #(friendsUI.friends or {}) == 0 then DrawWhiteInkText(cx, bodyTop + 80, "暂无好友")
        else
            for i, fr in ipairs(friendsUI.friends or {}) do
                ...
                nvgText(vg, pad + 14, y + 22, tostring(fr.nickname or ("玩家 " .. tostring(fr.userId or i))), nil)
                nvgText(vg, pad + 14, y + 44, "战力 " .. tostring(fr.combatPower or 0), nil)
                menuBtnRects["friendDel_" .. i] = { x = W - pad - 66, y = y + 17, w = 56, h = 30, userId = fr.userId }
                FriendBtn(W - pad - 66, y + 17, 56, 30, "删除", false)
            end
        end
    elseif friendsUI.tab == "add" then
        nvgText(vg, pad + 8, bodyTop + 4, "玩家ID:", nil)
        ...
        nvgText(vg, pad + 10, bodyTop + 40, friendsUI.searchId ~= "" and friendsUI.searchId or "点击输入", nil)
        menuBtnRects.friendSearchBtn = { x = W - pad - 70, y = bodyTop + 22, w = 70, h = 36 }
        FriendBtn(W - pad - 70, bodyTop + 22, 70, 36, "搜索", true)
        local y = bodyTop + 72
        if friendsUI.searchResult then
            ...
            nvgText(vg, pad + 14, y + 30, tostring(sr.nickname or ("玩家 " .. tostring(sr.userId or ""))), nil)
            if not (CloudManager.IsFriend and CloudManager.IsFriend(sr.userId)) then
                ...
                FriendBtn(W - pad - 66, y + 15, 56, 30, "添加", true)
            end
            y = y + 72
        elseif friendsUI.searchNotFound then
            DrawWhiteInkText(cx, y + 18, "未找到该玩家")
            y = y + 36
        end
        nvgText(vg, pad + 4, y + 4, "推荐:", nil)
        y = y + 26
        ...
        for i, rp in ipairs(friendsUI.recommended or {}) do
            ...
            nvgText(vg, pad + 12, cy + 27, tostring(rp.nickname or ("玩家 " .. tostring(rp.userId or ""))), nil)
            if not (CloudManager.IsFriend and CloudManager.IsFriend(rp.userId)) then
                ...
                FriendBtn(W - pad - 66, cy + 13, 56, 28, "添加", true)
            end
        end
    else
        ...
        if friendsUI.reqLoading then DrawWhiteInkText(cx, bodyTop + 80, "加载请求中...")
        elseif #(friendsUI.requests or {}) == 0 then DrawWhiteInkText(cx, bodyTop + 80, "暂无请求")
        else
            for i, req in ipairs(friendsUI.requests or {}) do
                ...
                nvgText(vg, pad + 14, y + 28, tostring(req.nickname or ("玩家 " .. tostring(req.fromUid or i))), nil)
                ...
                FriendBtn(W - pad - 122, y + 13, 52, 30, "同意", true)
                FriendBtn(W - pad - 56, y + 13, 52, 30, "拒绝", false)
            end
        end
    end
    ...
    DrawWhiteInkText(cx, py + 30, tostring(friendsUI.confirmPopup.title or "确认"))
    ...
    FriendBtn(cx - 120, py + ph - 50, 110, 38, "确认", true)
    FriendBtn(cx + 10, py + ph - 50, 110, 38, "取消", false)
end
