...
function DrawBattleButtons()
    ...
    if gameState.battlePhase == "TACTICS" and tacticState then
        ...
        -- [撤退] 按钮
        nvgBeginPath(vg); nvgRoundedRect(vg, bx, startY, btnW, btnH, 4)
        ...
        DrawWhiteInkText(bx + btnW / 2, startY + btnH / 2, "撤退")
        battleBackBtnRect = { x = bx, y = startY, w = btnW, h = btnH, isDesign = true }
        return
    end
    ...
    if isSHOP then
        ...
        if rawget(_G, "tbState") then
            ...
            -- [自动战斗] 按钮
            do
                local isAuto = gameSettings.tbAutoBattle
                ...
                nvgBeginPath(vg); nvgRoundedRect(vg, bx, curY, btnW, btnH, 4)
                nvgStrokeColor(vg, isAuto and nvgRGBA(80, 255, 140, math.floor(200 * autoPulse)) or nvgRGBA(130, 120, 110, 130))
                nvgStrokeWidth(vg, 1.0); nvgStroke(vg)
                nvgFontSize(vg, 20)
                nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
                DrawWhiteInkText(bx + btnW / 2, curY + btnH / 2, isAuto and "自动 ON" or "自动 OFF")
                tbAutoBattleBtnRect = { x = bx, y = curY, w = btnW, h = btnH, isDesign = true }
            end
            curY = curY + btnH + btnGap

            -- [结束回合] 按钮
            local canEnd = tbState.isPlayerTurn and tbState.animState == "idle" and not tbState.battleOver
            local endPulse = canEnd and (0.7 + 0.3 * math.sin(t * 3)) or 1.0
            nvgBeginPath(vg); nvgRoundedRect(vg, bx, curY, btnW, btnH, 4)
            if canEnd then
                nvgFillColor(vg, nvgRGBA(20, 60, 80, math.floor(220 * endPulse))); nvgFill(vg)
            else
                nvgFillColor(vg, nvgRGBA(35, 30, 25, 180)); nvgFill(vg)
            end
            nvgBeginPath(vg); nvgRoundedRect(vg, bx, curY, btnW, btnH, 4)
            nvgStrokeColor(vg, canEnd and nvgRGBA(80, 200, 255, math.floor(200 * endPulse)) or nvgRGBA(80, 70, 60, 100))
            nvgStrokeWidth(vg, 1.0); nvgStroke(vg)
            nvgFontSize(vg, 20)
            nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
            if canEnd then
                DrawWhiteInkText(bx + btnW / 2, curY + btnH / 2, "结束回合")
            else
                nvgFillColor(vg, nvgRGBA(120, 110, 100, 140))
                nvgText(vg, bx + btnW / 2, curY + btnH / 2, "结束回合", nil)
            end
            tbBtnEndTurn = canEnd and { x = bx, y = curY, w = btnW, h = btnH, isDesign = true } or nil
            return
        end
...
function DrawBottomActionBar()
    ...
    if gameState.battlePhase == "FIGHT" then
        ...
        local tbSize = math.floor(60 * btnSc)   -- 方形按钮边长 (放大, SLG风格)
        local tbGap = math.floor(10 * btnSc)    -- 按钮间距
        ...
        for i, tk in ipairs(troopKeys) do
            ...
            -- 兵种图标文字
            nvgFontSize(vg, math.floor(28 * btnSc))
            nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
            if hasUnits then
            ...
            nvgText(vg, bx + tbSize / 2, by + tbSize / 2 - 6, tt.icon, nil)
    ...
    -- 武技技能圈已移除 (武技改为全自动释放, 见 UpdateAutoSkills)
...
function DrawBattleRulesPopup()
    ...
    local ruleTitleSize = 18
    local ruleBodySize = 16
    local pw = 380
    local wrapW = pw - 48
...
-- 武技自动释放弹窗 (顶部横幅, 自动消失)
-- ============================================================================
function DrawSkillCastPopups()
    if #skillCastPopups == 0 then return end
    if fontId < 0 then return end
    ...
    local bannerH = 30
    local bannerGap = 4
    local startY = 60  -- 顶部偏移 (避开顶部 HUD)
    local cx = DESIGN_W / 2

    local i = 1
    while i <= #skillCastPopups do
        local p = skillCastPopups[i]
        p.timer = p.timer + dt

        if p.timer >= p.duration then
            table.remove(skillCastPopups, i)
        else
            -- 动画: 前0.3s淡入滑入, 后0.5s淡出
            local alpha = 1.0
            local slideY = 0
            if p.timer < 0.3 then
                local t = p.timer / 0.3
                alpha = t
                slideY = (1 - t) * (-15)
            elseif p.timer > p.duration - 0.5 then
                alpha = (p.duration - p.timer) / 0.5
            end

            local y = startY + (i - 1) * (bannerH + bannerGap) + slideY
            local c = p.color
            local a = math.floor(alpha * 255)

            -- 背景条
            nvgFontSize(vg, 16)
            local tw = nvgTextBounds(vg, 0, 0, p.text, nil)
            local bw = tw + 40
            nvgBeginPath(vg)
            nvgRoundedRect(vg, cx - bw / 2, y - bannerH / 2, bw, bannerH, 6)
            nvgFillColor(vg, nvgRGBA(c[1], c[2], c[3], math.floor(alpha * 50)))
            nvgFill(vg)
            nvgBeginPath(vg)
            nvgRoundedRect(vg, cx - bw / 2, y - bannerH / 2, bw, bannerH, 6)
            nvgStrokeColor(vg, nvgRGBA(c[1], c[2], c[3], math.floor(alpha * 120)))
            nvgStrokeWidth(vg, 1.0)
            nvgStroke(vg)
...
function DrawPhaseRulePopup()
    ...
    contentH = contentH + 40  -- 安全边距，确保内容可滚动
...
-- ============================================================================
-- 回合制战斗 - 底部操作面板 (选中兵团信息 + 武技按钮)
-- ============================================================================
function DrawTBBottomBar()
    ...
    local t = gameState.gameTime or 0
    local ts = tbState
    local isPlayerTurn = ts.isPlayerTurn
    local inAnim = (ts.animState ~= "idle")

    -- 底部面板
    local panelH = 80
    local panelY = DESIGN_H - panelH

    -- 选中兵团
    local selReg = nil
    if ts.selectedIdx and ts.selectedIdx > 0 and ts.selectedIdx <= #ts.playerRegiments then
        selReg = ts.playerRegiments[ts.selectedIdx]
    end
    ...
    -- === 左侧: 选中兵团信息 ===
    if selReg and selReg.alive then
        local infoX = 10
        local infoY = panelY + 6
        local tt = TROOP_TYPES[selReg.troopType]
        local c = tt and tt.color or { 200, 200, 200 }

        -- 兵团名 + 兵种
        nvgFontSize(vg, 20)
        nvgTextAlign(vg, NVG_ALIGN_LEFT + NVG_ALIGN_TOP)
        nvgFillColor(vg, nvgRGBA(c[1], c[2], c[3], 255))
        local nameStr = (selReg.heroName or "") .. " [" .. (tt and tt.name or "") .. "]"
        nvgText(vg, infoX, infoY, nameStr, nil)

        -- HP条
        local hpBarW = 120
        local hpBarH = 10
        local hpY = infoY + 22
        local hpRatio = selReg.hp / selReg.maxHP
        nvgBeginPath(vg); nvgRoundedRect(vg, infoX, hpY, hpBarW, hpBarH, 3)
        nvgFillColor(vg, nvgRGBA(30, 25, 20, 200)); nvgFill(vg)
        local hpR = hpRatio > 0.5 and 60 or (hpRatio > 0.25 and 220 or 255)
        local hpG = hpRatio > 0.5 and 200 or (hpRatio > 0.25 and 200 or 80)
        local hpB = hpRatio > 0.5 and 80 or (hpRatio > 0.25 and 60 or 60)
        nvgBeginPath(vg); nvgRoundedRect(vg, infoX, hpY, hpBarW * hpRatio, hpBarH, 3)
        nvgFillColor(vg, nvgRGBA(hpR, hpG, hpB, 220)); nvgFill(vg)

        -- HP数值 + 兵力
        nvgFontSize(vg, 16)
        nvgTextAlign(vg, NVG_ALIGN_LEFT + NVG_ALIGN_TOP)
        nvgFillColor(vg, nvgRGBA(200, 190, 180, 200))
        local atkLabel = selReg.atkRange > 1 and "远程" or "近战"
        local hpStr = string.format("HP:%d/%d 兵力:%d %s", math.floor(selReg.hp), math.floor(selReg.maxHP), selReg.unitCount, atkLabel)
        nvgText(vg, infoX, hpY + hpBarH + 3, hpStr, nil)

        -- ATK/DEF 数值
        nvgFontSize(vg, 14)
        nvgFillColor(vg, nvgRGBA(180, 170, 160, 180))
        local statStr = string.format("攻:%.0f 防:%.0f 移:%d 攻距:%d", selReg.atk, selReg.def, selReg.moveRange, selReg.atkRange)
        nvgText(vg, infoX, hpY + hpBarH + 19, statStr, nil)

        -- 状态标签
        if selReg.acted then
            nvgFontSize(vg, 16)
            nvgFillColor(vg, nvgRGBA(255, 140, 80, 200))
            nvgText(vg, infoX + hpBarW + 10, infoY, "已行动", nil)
        end
    end

    -- === 中间: 武技按钮 (每个有武技的己方兵团一个按钮) ===
    local skillBtns = {}
    for i, reg in ipairs(ts.playerRegiments) do
        if reg.alive and reg.card then
            -- 检查武将是否有装备武技
            local heroIdx = reg.heroIdx
            local techIdx = nil
            ...
            if techIdx and SKILL_TECHNIQUES and SKILL_TECHNIQUES[techIdx] then
                local canUse = isPlayerTurn and not inAnim and not ts.battleOver
                    and not reg.acted
                    and not ts.usedHeroes[heroIdx]
                    and ts.ap > 0
                table.insert(skillBtns, {
                    regIdx = i,
                    heroIdx = heroIdx,
                    techIdx = techIdx,
                    heroName = reg.heroName,
                    canUse = canUse,
                    troopColor = reg.troopColor,
                })
            end
        end
    end
    ...
    if #skillBtns > 0 then
        local btnSz = 48
        local btnGap = 12
        local totalW = #skillBtns * btnSz + (#skillBtns - 1) * btnGap
        local startX = (DESIGN_W - totalW) / 2
        local btnY = panelY + (panelH - btnSz) / 2

        for si, sb in ipairs(skillBtns) do
            local bx = startX + (si - 1) * (btnSz + btnGap)
            local tech = SKILL_TECHNIQUES[sb.techIdx]
            local tier = tech and SKILL_TIERS and SKILL_TIERS[tech.tier]
            local tc = tier and tier.color or { 180, 140, 255 }

            -- 按钮背景
            nvgBeginPath(vg); nvgRoundedRect(vg, bx, btnY, btnSz, btnSz, 6)
            if sb.canUse then
                local pulse = 0.6 + 0.4 * math.sin(t * 3 + si)
                nvgFillColor(vg, nvgRGBA(tc[1], tc[2], tc[3], math.floor(40 * pulse)))
            else
            ...
            local techName = tech and tech.name or "武技"
            -- 截取前2个字符
            local shortName = string.sub(techName, 1, 6) -- UTF-8 前2个汉字约6字节
            nvgText(vg, bx + btnSz / 2, btnY + 4, shortName, nil)

            -- 武将名
            nvgFontSize(vg, 12)
            nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_BOTTOM)
            local sc = sb.troopColor or { 200, 200, 200 }
            nvgFillColor(vg, nvgRGBA(sc[1], sc[2], sc[3], sb.canUse and 200 or 100))
            local heroShort = string.sub(sb.heroName or "", 1, 6)
            nvgText(vg, bx + btnSz / 2, btnY + btnSz - 2, heroShort, nil)

            -- 碰撞矩形
            if sb.canUse then
                tbSkillBtnRects[si] = {
                    x = bx, y = btnY, w = btnSz, h = btnSz, isDesign = true,
                    regIdx = sb.regIdx,
                }
            end
        end
    ...
    local apStr = "AP: " .. tostring(ts.ap) .. "/" .. tostring(TB_MAX_AP)
    nvgText(vg, DESIGN_W - 90, panelY + panelH - 24, apStr, nil)

    local turnStr = "第 " .. tostring(ts.turnNumber) .. " 回合"
    if not isPlayerTurn then
        turnStr = turnStr .. " - 敌方行动中"
    elseif ts.battleOver then
        turnStr = ts.winner == "player" and "我军胜利!" or "我军败北..."
    end
    nvgFontSize(vg, 16)
    nvgFillColor(vg, nvgRGBA(180, 170, 140, 200))
    nvgText(vg, DESIGN_W - 90, panelY + panelH - 6, turnStr, nil)
...
function DrawTacticActionBar()
    ...
    -- 判断当前状态: 是否玩家回合、是否有选中兵团、选中兵团是否已行动
    local isPlayerTurn = ts.isPlayerTurn
    local selGroup = nil
    if ts.selectedGroup and ts.selectedGroup > 0 and ts.selectedGroup <= #ts.playerGroups then
        selGroup = ts.playerGroups[ts.selectedGroup]
    end
    local hasSelection = (selGroup ~= nil and selGroup.alive)
    local hasActed = hasSelection and selGroup.acted
    local hasMoveTargets = hasSelection and ts.moveTargets and #ts.moveTargets > 0
    local hasAttackTargets = hasSelection and ts.attackTargets and #ts.attackTargets > 0
    local inAnim = (ts.animType ~= nil)

    -- 按钮尺寸和布局 (正方形, 不变形)
    local btnSz = 56
    local btnW, btnH = btnSz, btnSz
    local btnGap = 20
    local totalBtns = 4
    local totalW = totalBtns * btnSz + (totalBtns - 1) * btnGap
    local startX = (DESIGN_W - totalW) / 2
    local btnY = panelY + (panelH - btnSz) / 2

    -- 按钮定义: {key, label, imgKey, enabled}
    local buttons = {
        { key = "move",   label = "移动", imgKey = "btnMove",    enabled = isPlayerTurn and hasSelection and not hasActed and hasMoveTargets and not inAnim },
        { key = "attack", label = "攻击", imgKey = "btnAttack",  enabled = isPlayerTurn and hasSelection and not hasActed and hasAttackTargets and not inAnim },
        { key = "wait",   label = "待机", imgKey = "btnWait",    enabled = isPlayerTurn and hasSelection and not hasActed and not inAnim },
        { key = "end",    label = "结束",  imgKey = "btnEndTurn", enabled = isPlayerTurn and not inAnim },
    }
    ...
    for i, btn in ipairs(buttons) do
        local bx = startX + (i - 1) * (btnSz + btnGap)

        -- 按钮图片
        local imgHandle = nil
        if btn.enabled then
            imgHandle = IMG.tactic and IMG.tactic[btn.imgKey]
        else
            imgHandle = IMG.tactic and IMG.tactic.btnDisabled
        end

        if imgHandle then
            local alpha = btn.enabled and 1.0 or 0.5
            -- 选中状态脉冲 (移动/攻击按钮，当turnPhase匹配时高亮)
            if btn.enabled and (
                (btn.key == "move" and ts.turnPhase == "PLAYER_MOVE") or
                (btn.key == "attack" and ts.turnPhase == "PLAYER_ATTACK")
            ) then
                local pulse = 0.7 + 0.3 * math.sin(t * 5)
                alpha = pulse
            end
            local pat = nvgImagePattern(vg, bx, btnY, btnW, btnH, 0, imgHandle, alpha)
            nvgBeginPath(vg); nvgRoundedRect(vg, bx, btnY, btnW, btnH, 6)
            nvgFillPaint(vg, pat); nvgFill(vg)
        else
            -- fallback: 纯色按钮
            nvgBeginPath(vg); nvgRoundedRect(vg, bx, btnY, btnW, btnH, 6)
            if btn.enabled then
                nvgFillColor(vg, nvgRGBA(40, 55, 30, 220))
            else
                nvgFillColor(vg, nvgRGBA(35, 30, 25, 180))
            end
            nvgFill(vg)
            nvgBeginPath(vg); nvgRoundedRect(vg, bx, btnY, btnW, btnH, 6)
            nvgStrokeColor(vg, btn.enabled and nvgRGBA(120, 200, 80, 180) or nvgRGBA(80, 70, 60, 100))
            nvgStrokeWidth(vg, 1.0); nvgStroke(vg)
        end
        ...
        nvgText(vg, bx + btnW / 2, btnY + btnH / 2, btn.label, nil)
        ...
        if btn.enabled and (
            ...
            nvgBeginPath(vg); nvgRoundedRect(vg, bx - 1, btnY - 1, btnW + 2, btnH + 2, 7)
        ...
        -- 注册碰撞矩形
        if btn.enabled then
            local rect = { x = bx, y = btnY, w = btnW, h = btnH, isDesign = true }
            if btn.key == "move"   then tacticBtnMove   = rect end
            if btn.key == "attack" then tacticBtnAttack = rect end
            if btn.key == "wait"   then tacticBtnWait   = rect end
            if btn.key == "end"    then tacticBtnEnd    = rect end
        end
    end
    ...
    if hasSelection then
        ...
        local tt = TROOP_TYPES[g.troopType]
        local c = tt and tt.color or { 200, 200, 200 }
        ...
        nvgTextAlign(vg, NVG_ALIGN_LEFT + NVG_ALIGN_TOP)
        nvgFillColor(vg, nvgRGBA(c[1], c[2], c[3], 255))
        local nameStr = (g.heroName or "") .. " [" .. (tt and tt.name or "") .. "]"
        nvgText(vg, infoX, infoY, nameStr, nil)
        ...
        nvgTextAlign(vg, NVG_ALIGN_LEFT + NVG_ALIGN_TOP)
        nvgFillColor(vg, nvgRGBA(200, 190, 180, 200))
        local atkLabel = g.atkRange == 0 and "近战" or "远程"
        local hpStr = string.format("HP:%d/%d 兵力:%d %s", math.floor(g.hp), math.floor(g.maxHP), g.unitCount, atkLabel)
        nvgText(vg, infoX, hpY + hpBarH + 4, hpStr, nil)
    ...
    -- 右侧: 回合数和状态
    local turnStr = "第 " .. (ts.turnNumber or 1) .. " 回合"
    if not isPlayerTurn then
        turnStr = turnStr .. " - 敌方行动中"
    end
    nvgFontSize(vg, 18)
    nvgTextAlign(vg, NVG_ALIGN_RIGHT + NVG_ALIGN_BOTTOM)
    nvgFillColor(vg, nvgRGBA(180, 170, 140, 200))
    nvgText(vg, DESIGN_W - 90, panelY + panelH - 8, turnStr, nil)
