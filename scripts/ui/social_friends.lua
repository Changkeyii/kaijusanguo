...
function DrawPowerRankScreen()
    ...
    -- 4. 我的信息（页签下方: UID + 相关数据）
    local myInfoY = tabY + tabH + 16
    -- 获取 UID (CloudAPI 内部已含全部 fallback)
    local myUid = CloudAPI.GetUserId()
    local myUidStr = myUid ~= 0 and tostring(myUid) or "---"
    -- 左侧显示 UID
    nvgFontSize(vg, 16)
    nvgTextAlign(vg, NVG_ALIGN_LEFT + NVG_ALIGN_MIDDLE)
    nvgFillColor(vg, nvgRGBA(140, 135, 120, 180))
    nvgText(vg, 20, myInfoY, "UID:" .. myUidStr, nil)
    -- 中间显示对应tab信息
    nvgFontSize(vg, 22)
    nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
    if curTab == "power" then
        local myPower = CalcPlayerTotalPower()
        DrawWhiteInkText(cx, myInfoY, "我的战力: " .. FormatPower(myPower))
    elseif curTab == "faction" then
        ...
        DrawWhiteInkText(cx, myInfoY, "我的阵营: " .. fName .. " (Lv." .. (fLvInfo.level or 1) .. ")")
    else
        ...
        DrawWhiteInkText(cx, myInfoY, "我的职务: " .. myRoleRank)
    end
