...
-- 背包卡牌 (水墨风 + 命格标记)
function DrawInventoryCard(x, y, w, h, card, constellation, mergeable, hideName, containMode)
    ...
    -- 角色图
    local imgY = y + 3
    local imgH = containMode and (h - 10) or (h - 20)
    if GetHeroSheet(card) > 0 then
        local _sh, _sc, _sr = GetHeroSheetInfo(card)
        DrawCardImage(x + 3, imgY, w - 6, imgH, _sh, card.row, card.col, _sc, _sr, not containMode)
...
function DrawInfoPopup()
    ...
    local hasSealBonus = sb and (
        (sb.strAdd or 0) > 0 or (sb.intAdd or 0) > 0 or (sb.vitAdd or 0) > 0 or
        (sb.tecAdd or 0) > 0 or (sb.spdAdd or 0) > 0 or (sb.critRate or 0) > 0)
                ...
                if sealSlot then
                    ...
                    if effect then
                        ...
                        local line = {
                            slotIdx = i,
                            theme = effect.theme,
                            mainName = effect.mainName, mainVal = mainVal, mainUnit = mainUnit,
                            level = lv,
                        }
    ...
    if hasSealBonus then
        ...
        -- 加成后最终值行 (六德兵符: 五维间接影响 str→ATK, vit→HP)
        local s5 = card.stats5 or { str = 50, int = 50, vit = 50, tec = 50, spd = 50 }
        local sealStr = s5.str + (sb.strAdd or 0)
        local sealVit = s5.vit + (sb.vitAdd or 0)
        local fAtk = math.floor(rawAtk * (1 + sealStr * 0.003))
        local fDef = rawDef
        local fHp  = math.floor(rawHp  * (1 + sealVit * 0.003))
        nvgFontSize(vg, 22)
        nvgFillColor(vg, nvgRGBA(255, 120, 100, 240))
        local atkStr = "ATK:" .. fAtk
        if (sb.strAdd or 0) > 0 then atkStr = atkStr .. " 武+" .. sb.strAdd end
        nvgText(vg, axLx, ly, atkStr, nil)
        nvgFillColor(vg, nvgRGBA(100, 150, 255, 240))
        nvgText(vg, axLx + col3, ly, "DEF:" .. fDef, nil)
        nvgFillColor(vg, nvgRGBA(100, 230, 130, 240))
        local hpStr = "HP:" .. fHp
        if (sb.vitAdd or 0) > 0 then hpStr = hpStr .. " 体+" .. sb.vitAdd end
    ...
    if hasSealDetails then
        ...
        for _, sl in ipairs(sealLines) do
            ...
            if sl.mainName then
                local fmt = sl.mainUnit == "%" and "%.1f" or "%.0f"
                txt = txt .. sl.mainName .. "+" .. string.format(fmt, sl.mainVal) .. sl.mainUnit
            end
            txt = txt .. " L" .. sl.level
