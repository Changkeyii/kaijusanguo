...
function DrawRewardPopup()
    ...
    -- 存储尺寸供 input_move 拖拽滚动使用
    gameState.rewardContentH = contentH
    gameState.rewardListH = listH
        ...
        elseif rw.type == "equip" then
            ...
            if eqSet and eqPiece then
                ...
                for _, sd in ipairs(EQUIP_STAT_DEFS) do
                    local base = (eqPiece[sd.key] or 0)
                    if base > 0 then
                        table.insert(rwAttrParts, string.format("%s+%d%%", sd.label, math.ceil(base * tierMul)))
                    end
                end
                nvgText(vg, listX + listW - 12, iy + itemH / 2, table.concat(rwAttrParts, " "), nil)
            end
