...
function HitSlotCard(dx, dy)
    ...
    -- DEPLOY 阶段玩家卡牌有滚动偏移, 碰撞检测需要补偿
    local scrollOfs = 0
    if gameState.battlePhase == "DEPLOY" and rawget(_G, "deployCardScrollY") then
        scrollOfs = deployCardScrollY or 0
    end
    for i, slot in ipairs(PLAYER_SLOTS) do
        if slot.filled and slot.card then
            local scy = slot.cy - scrollOfs  -- 应用滚动偏移后的屏幕坐标
            if dx >= slot.cx - halfW and dx <= slot.cx + halfW and
               dy >= scy - halfH and dy <= scy + halfH then
                -- 确保点击在裁切区域内 (clipTop=20, clipBottom=480)
                if gameState.battlePhase == "DEPLOY" and (dy < 20 or dy > 480) then
                    -- 在裁切区域外, 不响应
                else
...
function HandleMouseWheel(eventType, eventData)
    ...
    elseif gameState.phase == "BATTLE" then
        -- DEPLOY 阶段: 鼠标在左侧卡牌区时滚动卡牌列表
        if gameState.battlePhase == "DEPLOY" and rawget(_G, "deployCardScrollY") then
            -- 获取鼠标设计坐标
            local mx = input:GetMousePosition().x
            local dmx = (mx - (offsetX or 0)) / (scale or 1)
            if dmx < 120 then
                deployCardScrollY = (deployCardScrollY or 0) - delta * 30
                deployCardScrollY = math.max(0, deployCardScrollY)
                return
            end
        end
        -- 战斗地图缩放 (滚轮向上=放大, 向下=缩小)
        battleZoom = battleZoom + delta * 0.25
        battleZoom = math.max(1.0, math.min(4.0, battleZoom))
        ClampBattlePan()
    end
end
...
function HandleMultiGesture(eventType, eventData)
    ...
    elseif gameState.phase == "BATTLE" then
        -- 战斗地图双指缩放 (张开=放大, 捏合=缩小)
        battleZoom = battleZoom + dDist * 5.0
        battleZoom = math.max(1.0, math.min(4.0, battleZoom))
        ClampBattlePan()
    end
end
...
function HandleTextInput(eventType, eventData)
    ...
    if gameState.phase == "GM_PANEL" then
        ...
        if GMPanel.IsActive() and GMPanel.handleTextInput(ch) then
            return
        end
    end
...
function HandleKeyDown(eventType, eventData)
    ...
    -- GM面板键盘处理 (最高优先级, 拦截所有按键)
    if gameState.phase == "GM_PANEL" then
        local GMPanel = require("ui.gm_panel")
        if GMPanel.IsActive() then
            GMPanel.handleKeyDown(key)
        end
        return
    end
