...
function DrawTradeScreen()
    ...
    if tradeState.tab == "market" then
        ...
        local listH = H - listTop - 10 - 38  -- 底部留 38px 给分页控件
        ...
        if #items == 0 then
            ...
            for i, item in ipairs(items) do
                ...
                if cellY + fullCellH > listTop - 10 and cellY < listTop + listH + 10 then
                    ...
                    local badgeW, badgeH = 24, 14
                    nvgBeginPath(vg)
                    nvgRoundedRect(vg, cellX + mCellSize - badgeW - 1, cellY + 1, badgeW, badgeH, 2)
                    ...
                    nvgText(vg, cellX + mCellSize - badgeW / 2 - 1, cellY + 1 + badgeH / 2, tierName, nil)
                    ...
                    nvgText(vg, cellX + 2, cellY + 2, lvStr, nil)
                    ...
                    if utf8.len(shortPiece) > 3 then shortPiece = string.sub(shortPiece, 1, utf8.offset(shortPiece, 4) - 1) .. ".." end
                    nvgText(vg, cellX + mCellSize / 2, infoY + 8, shortPiece, nil)
                    -- 价格
                    nvgFontSize(vg, 12); nvgFillColor(vg, nvgRGBA(255, 220, 100, 240))
        ...
        -- ======== 分页控件 (scissor 外绘制, 不被裁剪) ========
        local pgH = 32
        local pgY = H - 10 - pgH
        local pgBtnW = 72
        local pgGap = 8
        local curPage = TradeManager.state.marketPage or 1
        local hasNext = TradeManager.state.marketHasNextPage
        local hasPrev = curPage > 1

        -- 上一页按钮
        local prevX = cx - pgBtnW - pgGap / 2 - 60
        nvgBeginPath(vg); nvgRoundedRect(vg, prevX, pgY, pgBtnW, pgH, 6)
        nvgFillColor(vg, hasPrev and nvgRGBA(50, 70, 110, 220) or nvgRGBA(40, 40, 50, 150)); nvgFill(vg)
        if hasPrev then nvgStrokeColor(vg, nvgRGBA(80, 140, 200, 150)); nvgStrokeWidth(vg, 1); nvgStroke(vg) end
        nvgFontSize(vg, 18); nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
        nvgFillColor(vg, hasPrev and nvgRGBA(200, 220, 255, 255) or nvgRGBA(120, 120, 120, 150))
        nvgText(vg, prevX + pgBtnW / 2, pgY + pgH / 2, "< 上一页", nil)
        tradeState.btnRects.prevPage = { x = prevX, y = pgY, w = pgBtnW, h = pgH }

        -- 页码
        nvgFontSize(vg, 18); nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
        nvgFillColor(vg, nvgRGBA(200, 200, 200, 220))
        nvgText(vg, cx, pgY + pgH / 2, "第 " .. curPage .. " 页", nil)

        -- 下一页按钮
        local nextX = cx + pgGap / 2 + 60
        nvgBeginPath(vg); nvgRoundedRect(vg, nextX, pgY, pgBtnW, pgH, 6)
        nvgFillColor(vg, hasNext and nvgRGBA(50, 70, 110, 220) or nvgRGBA(40, 40, 50, 150)); nvgFill(vg)
        if hasNext then nvgStrokeColor(vg, nvgRGBA(80, 140, 200, 150)); nvgStrokeWidth(vg, 1); nvgStroke(vg) end
        nvgFontSize(vg, 18); nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
        nvgFillColor(vg, hasNext and nvgRGBA(200, 220, 255, 255) or nvgRGBA(120, 120, 120, 150))
        nvgText(vg, nextX + pgBtnW / 2, pgY + pgH / 2, "下一页 >", nil)
        tradeState.btnRects.nextPage = { x = nextX, y = pgY, w = pgBtnW, h = pgH }

    -- ======== 我的上架 Tab ========
    elseif tradeState.tab == "mine" then
        -- 裁剪区域 (防止内容溢出)
        nvgSave(vg)
        nvgScissor(vg, 0, bodyTop, W, bodyH)
        ...
        for _, item in ipairs(playerEquipment.owned) do
            if (item.tier or 0) >= 4 then
                ...
                for _, heroEquip in pairs(playerEquipment.equipped) do
                    if type(heroEquip) == "table" then
                        for _, eqUid in pairs(heroEquip) do
                            if eqUid == item.uid then isEquipped = true; break end
                        end
                    elseif heroEquip == item.uid then
                        isEquipped = true
                    end
        ...
        if #tradeableItems == 0 then
            ...
            for i, item in ipairs(tradeableItems) do
                ...
                local badgeW, badgeH = 24, 14
                nvgBeginPath(vg)
                nvgRoundedRect(vg, cx2 + tCellSize - badgeW - 1, cy2 + 1, badgeW, badgeH, 2)
                nvgFillColor(vg, nvgRGBA(tc[1], tc[2], tc[3], 180)); nvgFill(vg)
                nvgFontSize(vg, 12); nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
                nvgFillColor(vg, nvgRGBA(255, 255, 255, 240))
                nvgText(vg, cx2 + tCellSize - badgeW / 2 - 1, cy2 + 1 + badgeH / 2, tierName, nil)

                -- 强化角标
                if (item.enhanceLv or 0) > 0 then
                    nvgFontSize(vg, 11); nvgTextAlign(vg, NVG_ALIGN_LEFT + NVG_ALIGN_TOP)
                    nvgFillColor(vg, nvgRGBA(255, 220, 100, 220))
                    nvgText(vg, cx2 + 2, cy2 + 2, "+" .. item.enhanceLv, nil)
                end
                ...
                if utf8.len(shortP2) > 3 then shortP2 = string.sub(shortP2, 1, utf8.offset(shortP2, 4) - 1) .. ".." end
                nvgText(vg, cx2 + tCellSize / 2, infoY2 + 7, shortP2, nil)
                -- 强化+等级
                local lvP2 = {}
                if (item.enhanceLv or 0) > 0 then lvP2[#lvP2 + 1] = "+" .. item.enhanceLv end
                lvP2[#lvP2 + 1] = "Lv" .. (item.level or 1)
                nvgFontSize(vg, 11); nvgFillColor(vg, nvgRGBA(120, 220, 255, 210))
                nvgText(vg, cx2 + tCellSize / 2, infoY2 + 18, table.concat(lvP2, " "), nil)

                -- "上架" 按钮标签 (格子底部醒目提示)
                local listBtnH = 16
                nvgBeginPath(vg); nvgRoundedRect(vg, cx2 + 1, cy2 + tCellSize - listBtnH - 1, tCellSize - 2, listBtnH, 3)
                local listPulse = 0.8 + 0.2 * math.sin((menuAnimTimer or 0) * 2.5 + i)
                nvgFillColor(vg, nvgRGBA(math.floor(160 * listPulse), math.floor(100 * listPulse), 20, 220)); nvgFill(vg)
                nvgFontSize(vg, 12); nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
                nvgFillColor(vg, nvgRGBA(255, 255, 255, 255))
                nvgText(vg, cx2 + tCellSize / 2, cy2 + tCellSize - listBtnH / 2 - 1, "上架", nil)
        ...
        -- 更新最大滚动范围
        local mineContentH = curY - (bodyTop - scrollOff)
        if scrollOff > math.max(0, mineContentH - bodyH) then
            tradeState.scroll.offset = math.max(0, mineContentH - bodyH)
        end

        nvgRestore(vg)  -- 恢复 mine tab 的 scissor 裁剪
    end
            ...
            if pop.type == "buy" then
                ...
                if piece then
                    ...
                    for _, sd in ipairs(EQUIP_STAT_DEFS) do
                        ...
                        if base > 0 then
                            local val = sd.isCrit and (base * tierMul * enhMul)
                                or ((base + qBonus + lvBonus) * tierMul * enhMul)
                            table.insert(attrParts, string.format("%s+%.1f%%", sd.label, val))
                        end
                    end
...
function DrawTradeCard(x, y, w, h, item, idx, isMine)
    ...
    if piece then
        ...
        local qBonus = GetQualityBonus(eq.quality)
        local lvBonus = GetLevelBonus(eq.level)
        local tradeAttrParts = {}
        for _, sd in ipairs(EQUIP_STAT_DEFS) do
            local base = piece[sd.key] or 0
            if base > 0 then
                local val = sd.isCrit and (base * tierMul * enhMul)
                    or ((base + qBonus + lvBonus) * tierMul * enhMul)
                table.insert(tradeAttrParts, string.format("%s+%.1f%%", sd.label, val))
            end
        end
        nvgFontSize(vg, 20); nvgTextAlign(vg, NVG_ALIGN_LEFT + NVG_ALIGN_MIDDLE)
        nvgFillColor(vg, nvgRGBA(255, 160, 80, 220))
        nvgText(vg, x + 10, y + 68, table.concat(tradeAttrParts, "  "), nil)
    end
