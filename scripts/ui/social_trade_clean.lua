...
function DrawTradeScreen()
    ...
    tradeState.btnRects.back = { x = 10, y = 10, w = 100, h = 44 }; TradeBtn(10, 10, 100, 44, "< Back", false)
    DrawWhiteInkText(cx, 32, "Trade")
    ...
    TradeBtn(pad + 2, 56, tabW - 4, 36, "Market", tradeState.tab == "market")
    TradeBtn(pad + tabW + 2, 56, tabW - 4, 36, "Mine", tradeState.tab == "mine")
    local bodyTop = 102

    if tradeState.tab == "market" then
        tradeState.btnRects.refresh = { x = W - 94, y = bodyTop - 36, w = 80, h = 30 }
        TradeBtn(W - 94, bodyTop - 36, 80, 30, "Refresh", false)
        ...
        if TradeManager and TradeManager.state and TradeManager.state.marketLoading then DrawWhiteInkText(cx, bodyTop + 80, "Loading market...")
        elseif #items == 0 then DrawWhiteInkText(cx, bodyTop + 80, "No market items")
        else
            for i, item in ipairs(items) do
                ...
                nvgText(vg, pad + 12, y + 20, tostring(item.pieceName or item.name or item.listingKey or ("Item " .. i)), nil)
                nvgText(vg, pad + 12, y + 44, "Price: " .. tostring(item.price or 0), nil)
        ...
        TradeBtn(W - 136, bodyTop - 36, 122, 30, "Claim Jade", (myData.pendingJade or 0) > 0)
        nvgText(vg, pad, bodyTop - 20, "Pending Jade: " .. tostring(myData.pendingJade or 0), nil)
        ...
        for i, entry in ipairs(active) do
            ...
            nvgText(vg, pad + 12, y + 40, "Price: " .. tostring(listing.price or 0), nil)
            tradeState.btnRects["unlist_" .. i] = { x = W - pad - 78, y = y + 13, w = 68, h = 30 }
            TradeBtn(W - pad - 78, y + 13, 68, 30, "Unlist", false)
            y = y + 62
        end
        for i, key in ipairs(expired) do
            ...
            nvgText(vg, pad + 12, y + 30, "Expired: " .. tostring(key), nil)
            tradeState.btnRects["claim_" .. i] = { x = W - pad - 78, y = y + 13, w = 68, h = 30 }
            TradeBtn(W - pad - 78, y + 13, 68, 30, "Claim", true)
            y = y + 62
        end
        if y == bodyTop then DrawWhiteInkText(cx, bodyTop + 80, "No personal listings") end
    end
    ...
    DrawWhiteInkText(cx, py + 28, "Trade Confirm")
    local desc = pop.type or "action"
    if pop.type == "list" and pop.data then desc = "List price: " .. tostring(pop.data.price or 0) end
    ...
    TradeBtn(cx - 120, py + ph - 52, 110, 38, "Confirm", true)
    TradeBtn(cx + 10, py + ph - 52, 110, 38, "Cancel", false)
end
