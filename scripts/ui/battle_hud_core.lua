...
function HandleNanoVGRender(eventType, eventData)
    ...
    if gameState.phase == "LOADING" then
        ...
        nvgText(vg, cx + 2, cy - 35 + 2, "加载中", nil)
        -- 标题主层
        DrawWhiteInkText(cx, cy - 35, "加载中")
        ...
        nvgText(vg, cx, barY + barH + 32, "讨伐将启，请稍候嶅€?..", nil)
    ...
    elseif gameState.phase == "TD_SELECT" then
        ...
        TDSelect.Draw()
        DrawFloatTexts()
    elseif gameState.phase == "GM_PANEL" then
        local GMPanel = require("ui.gm_panel")
        GMPanel.Draw()
    elseif gameState.phase == "TD_BATTLE" then
        local TDRender = require("systems.td.td_render")
        TDRender.Draw()
        DrawFloatTexts()
        -- CSGO转盘奖励动画 (覆盖在战斗画面上方)
        local TDReward = require("systems.td.td_reward")
        if TDReward.IsActive() then
            TDReward.Draw()
        end
    else
        -- === 战场层 (受 battleZoom/Pan 影响) ===
        nvgSave(vg)
        do
            local cx, cy = DESIGN_W / 2, DESIGN_H / 2
            nvgTranslate(vg, cx, cy)
            nvgScale(vg, battleZoom, battleZoom)
            nvgTranslate(vg, -cx - battlePanX, -cy - battlePanY)
        end
        DrawBackground()
        local isTactics = (gameState.battlePhase == "TACTICS")
        ...
        DrawBattleUnits()  -- 内部已分支: TACTICS → DrawTacticGrid()
        if not isTactics then
            DrawSkillEffects()       -- 武技技能特效 (在单位之上)
            DrawSkillTargetingDesign()  -- 技能瞄准指示器
            DrawParticles()
            UpdateAndDrawProjectiles(time:GetTimeStep())
            DrawCardSlots()  -- 左右武将卡牌区 (DEPLOY阶段武将选择 + FIGHT阶段兵种显示)
            DrawPreBattleRoutes()   -- 战前路线选择 (SHOP阶段) / 战中路线指示 (FIGHT阶段)
            DrawDeployLaneHighlight()  -- DEPLOY阶段拖拽武将时车道高亮预览
        end
        DrawFloatTexts()
        nvgRestore(vg)  -- 恢复战场缩放变换

        -- === HUD层 (不受缩放影响, 固定在设计坐标) ===
        if not isTactics then
            DrawHUD()
            if gameState.battlePhase == "DEPLOY" then DrawDeployFormationPanel() end  -- 阵型切换面板
        end
        DrawBattleButtons()  -- 右侧按钮在设计坐标空间绘制 (撤退等)
        DrawBottomActionBar()  -- 战旗模式内部分支到 DrawTacticActionBar()
        if not isTactics then
            DrawSkillCastPopups() -- 武技自动释放弹窗
            DrawStrategyWheel()   -- 策略轮盘(长按自动行军弹出瑰嚭)
        end
        ...
        if not isTactics then
            ...
            DrawRetreatPursuitNotice()  -- 撤退被追击提示
            DrawEnemyRetreatPopup()     -- 敌方撤退弹窗(追击/放行)
            DrawPursuitResultNotice()   -- 追击结果通知
        end
    ...
    if gameState.isBanned then
        ...
        nvgFontSize(vg, 22); nvgFillColor(vg, nvgRGBA(220, 200, 200, 220))
        ...
        nvgFontSize(vg, 20); nvgFillColor(vg, nvgRGBA(150, 150, 160, 160))
        nvgText(vg, bcx, boxY + 130, "Use the community feedback channel if needed.", nil)
    end

    -- 屏幕转场遮罩 (设计坐标内, 最顶层)
    do
        local AnimLib = require("ui.anim")
        AnimLib.DrawTransition(DESIGN_W, DESIGN_H)
    end
...
local _ProceduralBG = require("ui.battle_bg_procedural")

function DrawBackground()
    -- ★ 战旗模式: 纯色深色背景，优先可见度
    if gameState.battlePhase == "TACTICS" then
        nvgBeginPath(vg); nvgRect(vg, 0, 0, DESIGN_W, DESIGN_H)
        nvgFillColor(vg, nvgRGBA(30, 35, 25, 255)); nvgFill(vg)
        return
    end
    ...
    if not bgHandle or not IsImageReady(bgHandle) then
        ...
        -- 无背景图: 绘制程序化 2.5D 地形
        _ProceduralBG.Draw(vg, DESIGN_W, DESIGN_H, gameState.gameTime or 0)
    else
        -- 有背景图: 拉伸/覆盖到设计分辨率 (默认战场为横屏水墨图; 讨伐层为旧竖屏拉伸)
...
function DrawCriticalLines()
    ...
    -- 敌方边界线 (横屏: 右侧垂直线, 古铜色)
    local ePulse = 0.3 + 0.2 * math.sin(t * 2)
    local eLineX = bz.enemyLine
    -- 渐变发光带
    local eGlow = nvgLinearGradient(vg, eLineX - 5, 0, eLineX + 5, 0,
        nvgRGBA(180, 160, 100, 0),
        nvgRGBA(180, 160, 100, math.floor(18 * ePulse)))
    nvgBeginPath(vg); nvgRect(vg, eLineX - 5, bz.top, 10, bz.bottom - bz.top)
    nvgFillPaint(vg, eGlow); nvgFill(vg)
    -- 主线
    nvgBeginPath(vg)
    nvgMoveTo(vg, eLineX, bz.top + 10); nvgLineTo(vg, eLineX, bz.bottom - 10)
    nvgStrokeColor(vg, nvgRGBA(180, 160, 100, math.floor(40 + 25 * ePulse)))
    nvgStrokeWidth(vg, 0.6); nvgStroke(vg)

    -- 玩家边界线 (横屏: 左侧垂直线, 古铜色)
    local pPulse = 0.3 + 0.2 * math.sin(t * 2 + 1.5)
    local pLineX = bz.playerLine
    -- 渐变发光带
    local pGlow = nvgLinearGradient(vg, pLineX - 5, 0, pLineX + 5, 0,
        nvgRGBA(180, 160, 100, math.floor(18 * pPulse)),
        nvgRGBA(180, 160, 100, 0))
    nvgBeginPath(vg); nvgRect(vg, pLineX - 5, bz.top, 10, bz.bottom - bz.top)
    nvgFillPaint(vg, pGlow); nvgFill(vg)
    -- 主线
    nvgBeginPath(vg)
    nvgMoveTo(vg, pLineX, bz.top + 10); nvgLineTo(vg, pLineX, bz.bottom - 10)
    nvgStrokeColor(vg, nvgRGBA(180, 160, 100, math.floor(40 + 25 * pPulse)))
    nvgStrokeWidth(vg, 0.6); nvgStroke(vg)
end
...
-- 战前准备页(DEPLOY)卡牌区滚动偏移
deployCardScrollY = 0

function DrawCardSlots()
    -- FIGHT阶段隐藏所有武将卡牌 (战场上只看兵种单位)
    if gameState.battlePhase == "FIGHT" then return end
    -- SLG世界地图战斗: 武将已预设, 不显示卡牌选择UI
    if gameState.worldMapBattle then return end

    -- DEPLOY 阶段: 添加 scissor 裁切, 防止卡牌与底部按钮重叠
    local isDeployClip = (gameState.battlePhase == "DEPLOY")
    -- 裁切区域: 左侧卡牌列 x=0~120, 顶部留 20px, 底部留出按钮区域 (到 y=480)
    local clipLeft = 0
    local clipTop = 20
    local clipRight = 120
    local clipBottom = 480  -- 低于此区域的内容被裁切 (按钮在 ~486+)
    -- 右侧敌方卡牌列
    local clipEnemyLeft = 900
    local clipEnemyRight = DESIGN_W

    if isDeployClip then
        -- 计算卡牌内容总高度
        local slotCount = #PLAYER_SLOTS
        if slotCount > 0 then
            local firstY = PLAYER_SLOTS[1].cy - SLOT_CARD_H / 2
            local lastY = PLAYER_SLOTS[slotCount].cy + SLOT_CARD_H / 2 + 16 -- 加余量(车道标签)
            local contentH = lastY - firstY
            local viewH = clipBottom - clipTop
            local maxScroll = math.max(0, contentH - viewH)
            deployCardScrollY = math.max(0, math.min(deployCardScrollY, maxScroll))
        end
    end
    ...
    if isDeployClip then
        nvgSave(vg)
        nvgScissor(vg, clipEnemyLeft, clipTop, clipEnemyRight - clipEnemyLeft, clipBottom - clipTop)
    end
    for _, slot in ipairs(ENEMY_SLOTS) do DrawSlotCard(slot, false) end
    if isDeployClip then nvgRestore(vg) end

    -- 玩家卡牌 (左侧, 支持滚动 + 裁切)
    if isDeployClip then
        nvgSave(vg)
        nvgScissor(vg, clipLeft, clipTop, clipRight - clipLeft, clipBottom - clipTop)
        nvgTranslate(vg, 0, -deployCardScrollY)
    end
    for _, slot in ipairs(PLAYER_SLOTS) do
        if isDeployClip then
            -- 将槽位的碰撞区域也偏移 (供输入检测用)
            slot._renderOffsetY = -deployCardScrollY
        end
        DrawSlotCard(slot, true)
    end
    if isDeployClip then
        nvgRestore(vg)
        -- 绘制滚动指示器 (如果内容可滚动)
        ...
        if slotCount > 0 then
            local firstY = PLAYER_SLOTS[1].cy - SLOT_CARD_H / 2
            local lastY = PLAYER_SLOTS[slotCount].cy + SLOT_CARD_H / 2 + 16
            local contentH = lastY - firstY
            local viewH = clipBottom - clipTop
            if contentH > viewH then
                local scrollBarH = math.max(20, viewH * viewH / contentH)
                local scrollTrackH = viewH - scrollBarH
                local scrollBarY = clipTop + (deployCardScrollY / (contentH - viewH)) * scrollTrackH
                nvgBeginPath(vg)
                nvgRoundedRect(vg, clipRight - 4, scrollBarY, 3, scrollBarH, 1.5)
                nvgFillColor(vg, nvgRGBA(200, 190, 170, 100))
                nvgFill(vg)
            end
        end
        -- 清理渲染偏移标记
        for _, slot in ipairs(PLAYER_SLOTS) do slot._renderOffsetY = nil end
    end
end
...
function DrawSlotCard(slot, isPlayer)
    ...
    if slot.filled and slot.card then
        ...
        -- ======== FIGHT阶段: 简化兵种卡牌 (武将固定, 不可拖拽) ========
        if gameState.battlePhase == "FIGHT" and isPlayer then
            local uc = UNIT_CLASS and UNIT_CLASS[card.unitClass]
            local ucName = uc and uc.name or "兵"
            local qc = QUALITY_COLORS[card.quality] or { 200, 195, 180 }

            -- 半透明兵种底板
            nvgBeginPath(vg); nvgRoundedRect(vg, cx - w/2, cy - h/2, w, h, 4)
            nvgFillColor(vg, nvgRGBA(8, 14, 24, 160)); nvgFill(vg)
            ...
            if fontId >= 0 then
                ...
                nvgFontSize(vg, 20)
                nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
                DrawWhiteInkText(cx, cy + 2, string.sub(ucName, 1, 3))
            end
            ...
            if fontId >= 0 then
                ...
                if sc > 0 then
                    ...
                    nvgFill(vg)
                    nvgStrokeColor(vg, nvgRGBA(200, 255, 200, 180)); nvgStrokeWidth(vg, 1.2); nvgStroke(vg)
                    nvgFontSize(vg, 18)
                    nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
            ...
            -- 部署冷却覆盖层 (已移除: 新系统使用一次性预部署，无需冷却倒计时)
            return
        end
        ...
        -- === 车道指示器 (DEPLOY阶段, 玩家卡片, 卡牌下方) ===
        if isPlayer and fontId >= 0 and gameState.battlePhase == "DEPLOY" and slot.deployLane then
            local laneNum = slot.deployLane
            local lnW, lnH = 18, 11
            local lnX = cx - lnW / 2
            local lnY = cy + h / 2 + 2
            -- 车道颜色: 每路不同色调
            ...
            local lc = laneColors[laneNum] or { 180, 175, 165 }
            nvgBeginPath(vg); nvgRoundedRect(vg, lnX, lnY, lnW, lnH, 2)
            nvgFillColor(vg, nvgRGBA(lc[1], lc[2], lc[3], 200)); nvgFill(vg)
            nvgStrokeColor(vg, nvgRGBA(255, 255, 255, 100))
            nvgStrokeWidth(vg, 0.6); nvgStroke(vg)
            nvgFontFaceId(vg, GetMainFont()); nvgFontSize(vg, 20)
            nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
            DrawWhiteInkText(lnX + lnW / 2, lnY + lnH / 2, laneNum .. "路")
            -- 存储点击区域供输入检测 (挂在slot上)
            slot.laneBadgeRect = { x = lnX, y = lnY, w = lnW, h = lnH, isDesign = true }
        end
...
-- 拖拽时的车道高亮预览 (DEPLOY阶段, 拖拽武将到战场时显示目标车道)
function DrawDeployLaneHighlight()
    if not dragState.active or not dragState.fromSlot then return end
    if gameState.battlePhase ~= "DEPLOY" then return end
    local ddx, ddy = LogicalToDesign(dragState.lx, dragState.ly)
    local bz = BATTLE_ZONE
    if ddy < bz.top or ddy > bz.bottom or ddx < bz.playerLine or ddx > bz.enemyLine then return end
    -- 检测车道
    local laneIdx = math.floor((ddy - bz.top) / LANE_WIDTH) + 1
    laneIdx = math.max(1, math.min(NUM_LANES, laneIdx))
    local laneY = bz.top + (laneIdx - 1) * LANE_WIDTH
    local laneColors = {
        { 80, 180, 255 }, { 100, 220, 120 }, { 255, 210, 60 },
        { 255, 150, 60 }, { 220, 100, 180 },
    }
    local lc = laneColors[laneIdx] or { 180, 175, 165 }
    local t = gameState.gameTime
    local pulse = 0.6 + 0.4 * math.sin(t * 5)
    -- 车道高亮条
    nvgBeginPath(vg)
    nvgRect(vg, bz.playerDeployLeft, laneY, bz.playerDeployRight - bz.playerDeployLeft, LANE_WIDTH)
    nvgFillColor(vg, nvgRGBA(lc[1], lc[2], lc[3], math.floor(35 * pulse)))
    nvgFill(vg)
    -- 车道边框
    nvgBeginPath(vg)
    nvgRect(vg, bz.playerDeployLeft, laneY, bz.playerDeployRight - bz.playerDeployLeft, LANE_WIDTH)
    nvgStrokeColor(vg, nvgRGBA(lc[1], lc[2], lc[3], math.floor(120 * pulse)))
    nvgStrokeWidth(vg, 1.5); nvgStroke(vg)
    -- 车道标签
    nvgFontFaceId(vg, GetMainFont())
    nvgFontSize(vg, 28)
    nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
    nvgFillColor(vg, nvgRGBA(lc[1], lc[2], lc[3], math.floor(200 * pulse)))
    nvgText(vg, (bz.playerDeployLeft + bz.playerDeployRight) / 2,
        laneY + LANE_WIDTH / 2, laneIdx .. "路", nil)
end

-- ============================================================================
-- 战前路线选择 / 战中路线指示  (SHOP阶段: 可交互; FIGHT阶段: 仅显示)
-- ============================================================================
function DrawPreBattleRoutes()
    if fontId < 0 then return end
    local bz = BATTLE_ZONE
    local isSHOP = (gameState.battlePhase == "DEPLOY")
    local isFIGHT = (gameState.battlePhase == "FIGHT")
    if not isSHOP and not isFIGHT then return end
    ...
    if isSHOP then
        for lane = 1, NUM_LANES do
            local laneY = bz.top + (lane - 1) * LANE_WIDTH
            local lc = laneColors[lane]
            local pulse = 0.3 + 0.15 * math.sin(t * 2 + lane * 0.8)
            nvgBeginPath(vg)
            nvgRect(vg, bz.playerLine + 4, laneY, bz.enemyLine - bz.playerLine - 8, LANE_WIDTH - 1)
            nvgFillColor(vg, nvgRGBA(lc[1], lc[2], lc[3], math.floor(20 * pulse))); nvgFill(vg)
        end

        -- 顶部提示文字
        nvgFontSize(vg, 20)
        nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_TOP)
        local hintA = math.floor(140 + 60 * math.sin(t * 2.0))
        nvgFillColor(vg, nvgRGBA(255, 220, 130, hintA))
        nvgText(vg, DESIGN_W / 2, bz.top + 6, "点击武将→选择行军路线  未分配=均匀五路", nil)
    end

    -- 绘制每个玩家武将的路线分配标签
    for i, slot in ipairs(PLAYER_SLOTS) do
        if slot.filled then
            local laneNum = heroLaneAssignment[i]
            local isSelecting = (preBattleSelectingSlot == i)

            if isSHOP then
                -- 选中状态: 外圈高亮脉冲
                if isSelecting then
                    local sel_pulse = 0.6 + 0.4 * math.sin(t * 6)
                    nvgBeginPath(vg); nvgCircle(vg, slot.cx, slot.cy, SLOT_CARD_W / 2 + 6)
                    nvgStrokeColor(vg, nvgRGBA(255, 230, 80, math.floor(220 * sel_pulse)))
                    nvgStrokeWidth(vg, 2.5); nvgStroke(vg)
                end

                -- 路线标签 (卡牌右上角小圆角框)
                local lblX = slot.cx + SLOT_CARD_W / 2 + 2
                local lblY = slot.cy - SLOT_CARD_H / 2
                local lblW, lblH = 22, 14
                nvgBeginPath(vg); nvgRoundedRect(vg, lblX, lblY, lblW, lblH, 3)
                if laneNum then
                    local lc = laneColors[laneNum]
                    nvgFillColor(vg, nvgRGBA(lc[1], lc[2], lc[3], 200)); nvgFill(vg)
                    nvgFontSize(vg, 20)
                    nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
                    DrawWhiteInkText(lblX + lblW / 2, lblY + lblH / 2, tostring(laneNum) .. "路")
                else
                    nvgFillColor(vg, nvgRGBA(80, 80, 100, 160)); nvgFill(vg)
                    nvgFontSize(vg, 18)
                    nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
                    DrawWhiteInkText(lblX + lblW / 2, lblY + lblH / 2, "均")
                end

                -- 选中状态: 弹出5个车道选择按钮 (在武将右侧横排)
                if isSelecting then
                    local btnR = 14
                    local startBx = bz.playerLine + 20
                    local btnCY = slot.cy
                    for lane = 1, NUM_LANES do
                        local lc = laneColors[lane]
                        local bx2 = startBx + (lane - 1) * (btnR * 2 + 5)
                        local by2 = btnCY
                        -- 按钮主体
                        nvgBeginPath(vg); nvgCircle(vg, bx2, by2, btnR)
                        if laneNum == lane then
                            nvgFillColor(vg, nvgRGBA(lc[1], lc[2], lc[3], 220)); nvgFill(vg)
                        else
                            nvgFillColor(vg, nvgRGBA(lc[1], lc[2], lc[3], 80)); nvgFill(vg)
                        end
                        nvgStrokeColor(vg, nvgRGBA(lc[1], lc[2], lc[3], 200))
                        nvgStrokeWidth(vg, 1.5); nvgStroke(vg)
                        -- 路线数字
                        nvgFontSize(vg, 22)
                        nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
                        DrawWhiteInkText(bx2, by2, tostring(lane))
                        -- 存储按钮碰撞区 (用于点击检测 → 在 input 模块处理)
                        if rawget(_G, "preBattleLaneBtnRects") then
                            preBattleLaneBtnRects[lane] = { cx = bx2, cy = by2, r = btnR, isDesign = true }
                        end
                    end
                end
            end

            -- FIGHT阶段: 在武将旁显示小色条标示路线
            if isFIGHT and laneNum then
                local lc = laneColors[laneNum]
                local barX = slot.cx + SLOT_CARD_W / 2 + 3
                local laneY = bz.top + (laneNum - 1) * LANE_WIDTH + LANE_WIDTH / 2
                -- 从武将指向目标路线的连接线
                nvgBeginPath(vg)
                nvgMoveTo(vg, barX, slot.cy)
                nvgLineTo(vg, bz.playerLine + 12, laneY)
                nvgStrokeColor(vg, nvgRGBA(lc[1], lc[2], lc[3], 80))
                nvgStrokeWidth(vg, 1.0); nvgStroke(vg)
            end
        end
    end
...
-- 战前路线按钮碰撞区 (全局, 供 input_begin_press 使用)
preBattleLaneBtnRects = {}
...
function DrawToast(W, H)
    ...
    local now = (rawget(_G, "gameState") and gameState.gameTime) or 0
    local elapsed = now - (toastState.startTime or 0)
    local slideInDur = 0.3
    local slideOutDur = 0.3

    -- 滑入(前0.3s) / 持续 / 滑出(最后0.3s)
    local slideY, alpha
    if elapsed < slideInDur then
        local p = Anim.easeOutCubic(elapsed / slideInDur)
        slideY = -44 + 44 * p
        alpha = p
    elseif toastState.timer < slideOutDur then
        local p = toastState.timer / slideOutDur
        slideY = -16 * (1 - p)
        alpha = p
    else
        slideY = 0
    ...
    local cat = toastState.category or "info"
    local colors = Anim.toastColors[cat] or Anim.toastColors.info

    nvgSave(vg)
    nvgFontFaceId(vg, GetMainFont())
    nvgFontSize(vg, 18)
    nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
    local tw = nvgTextBounds(vg, 0, 0, msg, nil) + 40
    local th = 36
    local tx = W / 2 - tw / 2
    local ty = 18 + slideY
    ...
    -- 阴影
    nvgBeginPath(vg)
    nvgRoundedRect(vg, tx + 1, ty + 2, tw, th, 8)
    nvgFillColor(vg, nvgRGBA(0, 0, 0, math.floor(alpha * 60)))
    nvgFill(vg)
    -- 背景
    nvgBeginPath(vg)
    nvgRoundedRect(vg, tx, ty, tw, th, 8)
    nvgFillColor(vg, nvgRGBA(colors.bg[1], colors.bg[2], colors.bg[3], a))
    nvgFill(vg)
    nvgStrokeColor(vg, nvgRGBA(colors.fg[1], colors.fg[2], colors.fg[3], math.floor(alpha * 80)))
    nvgStrokeWidth(vg, 1); nvgStroke(vg)
    -- 左侧色条
    nvgBeginPath(vg)
    nvgRoundedRect(vg, tx + 4, ty + 7, 3, th - 14, 1.5)
    nvgFillColor(vg, nvgRGBA(colors.fg[1], colors.fg[2], colors.fg[3], a))
    nvgFill(vg)
    -- 文字
    nvgFillColor(vg, nvgRGBA(colors.fg[1], colors.fg[2], colors.fg[3], a))
    nvgText(vg, W / 2 + 4, ty + th / 2, msg, nil)
    nvgRestore(vg)
end
