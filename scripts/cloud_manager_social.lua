...
function CloudManager._publishProfile(allData)
    if not CWP.IsAvailable() then return end
    -- 封禁检查
    ...
    -- CWP: 公开资料 + 战力排行
    CWP.Write({
        { key = KEYS.pub_profile, value = profile },
        { key = KEYS.combat_power, value = combatPower, int = true },
        { key = KEYS.realm_level, value = pi.rankIdx or 1, int = true },
    }, "发布公开档案")
end
...
function CloudManager._publishOutbox()
    ...
    _purgeExpired(CloudManager._outgoingRequests)
    CWP.Write({
        { key = KEYS.freq_outbox_ts, value = os.time(), int = true },
        { key = KEYS.freq_outbox, value = CloudManager._outgoingRequests },
    }, "发布好友申请出站")
end
...
function CloudManager._publishResponses()
    ...
    _purgeExpired(CloudManager._outgoingResponses)
    CWP.Write({
        { key = KEYS.freq_resp_ts, value = os.time(), int = true },
        { key = KEYS.freq_resp, value = CloudManager._outgoingResponses },
    }, "发布好友回复")
end
...
function CloudManager.DonateFaction(amount, callback)
    ...
    -- 服务端权威模式：扣款走 RPC，成功后再写 camp_meta
    if rawget(_G, "cl_state") then
        ...
        ClientNet.Request("faction_donate", {
            ...
            if not meta.donateDaily[uidStr2] then meta.donateDaily[uidStr2] = { day = today2, amount = 0 } end
            if meta.donateDaily[uidStr2].day ~= today2 then
                meta.donateDaily[uidStr2] = { day = today2, amount = 0 }
            end
            meta.exp = (meta.exp or 0) + amount
            meta.funds = (meta.funds or 0) + amount
            meta.donateDaily[uidStr2].amount = (meta.donateDaily[uidStr2].amount or 0) + amount
            if not meta.contributions then meta.contributions = {} end
            meta.contributions[uidStr2] = (meta.contributions[uidStr2] or 0) + amount
            -- 检查升级
            local oldLevel2 = meta.level or 1
            local newLevel2 = oldLevel2
            for lv = oldLevel2 + 1, (rawget(_G, "FACTION_MAX_LEVEL") or 10) do
                if meta.exp >= (FACTION_LEVEL_EXP[lv] or 999999999) then
                    newLevel2 = lv
                else break end
            end
            local leveled2 = newLevel2 > oldLevel2
            meta.level = newLevel2
            if leveled2 then
                meta.maxMembers = FACTION_LEVEL_MAX_MEMBERS[newLevel2] or meta.maxMembers
            end
            CWP.Write({
                { key = KEYS.camp_meta, value = meta },
            }, "阵营捐献(RPC)", {
                ok = function()
                    ...
                    CWP.SetInt(rankKey, rankScore, "阵营等级排行")
                    if rawget(_G, "SaveGameProgress") then SaveGameProgress() end
                    if callback then callback(true, leveled2 and ("阵营升级到Lv." .. newLevel2 .. "!") or nil) end
                end,
                error = function(e)
                    if callback then callback(false, "camp_meta保存失败: " .. tostring(e)) end
                end,
            })
        end)
...
function CloudManager._syncSocialDomain()
    ...
    CWP.Set(DOMAINS.social, data, "social域同步", {
        ok = function()
            print("[社交] social域已同步")
        end,
    })
...
function CloudManager.CreateFaction(name, desc, callback)
    ...
    -- 写 camp_meta 的共享逻辑
    local function _publishFaction(finalJade)
        ...
        CWP.Write({
            { key = KEYS.camp_leader_ts, value = ts, int = true },
            { key = KEYS.camp_meta, value = meta },
        }, "创建阵营", {
            ok = function()
            ...
            error = function(_, reason)
                -- 回滚本地状态（服务端已扣款不回滚，但阵营创建失败需要恢复阵营状态）
                CloudManager._factionId = 0
                CloudManager._factionName = ""
    ...
    -- 单机模式：本地扣虎符
    _publishFaction(playerInfo.jade - CAMP_CREATE_COST)
end
...
function CloudManager.ApplyToFaction(campId, campName, callback)
    ...
    CWP.Write({
        { key = KEYS.camp_apply_ts, value = os.time(), int = true },
        { key = KEYS.camp_apply, value = apply },
    }, "申请加入阵营", {
        ok = function()
...
function CloudManager.ApproveFactionApplication(applicantUserId, callback)
    ...
    -- CWP: 更新 camp_meta + 发布审批回复
    CWP.Write({
        { key = KEYS.camp_meta, value = meta },
        { key = KEYS.camp_resp_ts, value = os.time(), int = true },
        { key = KEYS.camp_resp, value = CloudManager._campOutResp },
    }, "同意入营", {
        ok = function()
...
function CloudManager.LeaveFaction(callback)
    ...
    if wasLeader and meta then
        ...
        if successor then
            ...
            CWP.Write({
                { key = KEYS.camp_meta, value = meta },
                { key = KEYS.camp_resp_ts, value = 0, int = true },
                { key = KEYS.camp_resp, value = {} },
            }, "盟主退位, 转让给" .. tostring(successor), {
                ok = function()
            ...
            CWP.Write({
                { key = KEYS.camp_leader_ts, value = 0, int = true },
                { key = KEYS.camp_meta, value = {} },
                { key = KEYS.camp_resp_ts, value = 0, int = true },
                { key = KEYS.camp_resp, value = {} },
            }, "解散阵营(最后一人)", {
                ok = function()
