...
function CloudAPI.GetUserId()
    local uid = tonumber(CloudAPI.userId) or 0
    -- fallback: netState (服务端 Welcome 事件设置)
    if uid == 0 and rawget(_G, "netState") then
        local nsUid = tonumber(netState.userId) or 0
        if nsUid ~= 0 then
            uid = nsUid
            CloudAPI.userId = uid  -- 缓存供后续使用
        end
    end
    return uid
end
...
function CloudAPI._OnServerReady(netSt)
    ...
    -- 重置排行榜标志位，让 auto-retry 重新拉取数据
    local ws = rawget(_G, "welfareState")
    if ws then
        ws.powerLoaded = false;  ws.powerLoading = false
        ws.realmLoaded = false;  ws.realmLoading = false
        ws.factionRankLoaded = false; ws.factionRankLoading = false
    end
    -- 爬塔/排位排行榜已移除，无需重置
