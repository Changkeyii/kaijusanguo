...
function LoadSettings()
    ...
    if file:IsOpen() then
        ...
        if ok and data then
            ...
            -- 精灵上限
            gameSettings.spriteLimit = data.spriteLimit or 1000
            -- 回合制自动战斗
            gameSettings.tbAutoBattle = data.tbAutoBattle or false
        end
    end
    -- 应用音量
...
function ApplySaveData(data)
    ...
    if data.playerEquipment then
        ...
        -- ========== 迁移: equipped 扁平格式 → 按武将嵌套格式 ==========
        -- 旧存档: equipped = {[slotIdx]=uid, ...}  (值是number)
        -- 新存档: equipped = {[heroIdx]={[slotIdx]=uid, ...}}  (值是table)
        local flatEntries = {}
        local nestedEntries = {}
        local hasFlat = false
        for k, v in pairs(playerEquipment.equipped) do
        ...
        if hasFlat then
            ...
            for idx, hero in pairs(playerHeroes) do
                if hero.owned then
                    if not defaultHero or idx < defaultHero then
                        defaultHero = idx
                    end
                end
            end
            ...
            playerEquipment.equipped[defaultHero] = flatEntries
            for k, v in pairs(nestedEntries) do
                playerEquipment.equipped[k] = v
            end
            print("[存档] equipped迁移: 扁平→按武将, 默认分配给武将#" .. tostring(defaultHero))
        end
        -- 确保所有 equipped 引用的 item 都有正确的 heroIdx
        for heroIdx, heroEquip in pairs(playerEquipment.equipped) do
            if type(heroEquip) == "table" then
                for slotIdx, uid in pairs(heroEquip) do
                    for _, item in ipairs(playerEquipment.owned) do
    ...
    -- ========== 迁移: playerEquippedSkills 全局数组 → 按武将嵌套格式 ==========
    -- 旧格式: {skillIdx1, skillIdx2} (纯数组, 键为1,2)
    -- 新格式: {[heroIdx] = {skillIdx1, skillIdx2}}
    if playerEquippedSkills and #playerEquippedSkills > 0 then
        -- 检测: 如果 [1] 是数字(skillIdx), 说明是旧格式
        local firstVal = playerEquippedSkills[1]
        if type(firstVal) == "number" then
            local oldSkills = {}
            for _, si in ipairs(playerEquippedSkills) do
                oldSkills[#oldSkills + 1] = si
            end
            -- 找默认武将 (最小的已拥有 heroIdx)
            local defaultHero = nil
            for hIdx, hero in pairs(playerHeroes) do
                if hero.owned then
                    if not defaultHero or hIdx < defaultHero then
                        defaultHero = hIdx
                    end
                end
            end
            ...
            print("[存档] playerEquippedSkills迁移: 全局数组→按武将, 默认分配给武将#" .. tostring(defaultHero))
        end
    end
