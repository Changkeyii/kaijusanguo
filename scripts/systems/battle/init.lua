...
function ArrangeUnitsInFormation(units, isPlayer)
    ...
    -- 获取当前阵型的兵种区域配置 (玩家侧优先使用手动交换后的自定义区域)
    ...
    local zones
    if isPlayer and deployCustomZones then
        zones = deployCustomZones
    else
        zones = (form and form.troopZones) or DEFAULT_TROOP_ZONES
    end
...
function InitBattle()
    ...
    floatTexts = {}
    particles = {}
    -- 默认不使用战旗模式（如需战旗，调用方应在 InitBattle() 前设 gameState.useTacticsMode = true）
    if gameState.useTacticsMode == nil then
    ...
    -- 重置战斗地图缩放/平移
    battleZoom = 1.0   -- 默认最小缩放=俯视全景
    battlePanX = 0
    battlePanY = 0
    -- ★ 程序化地形种子 (每场战斗不同)
    _ProceduralBG.SetSeed(nil)
    _battlePanning = false
    skillCastPopups = {}
    deploySwapFirstTroop = nil
    deployCustomZones = nil
    inventory = {}
    invScrollOffset = 0
    shopCards = {}
    ...
    -- ★ 应用战斗布局: 从 BATTLE_LAYOUTS 中随机选择背景
    local layoutCount = #BATTLE_LAYOUTS
    local bgIdx = math.random(1, layoutCount)
    ApplyBattleLayout(bgIdx)
    print("[InitBattle] 随机战场背景: " .. (BATTLE_LAYOUTS[bgIdx] and BATTLE_LAYOUTS[bgIdx].name or bgIdx))
    ...
    if rawget(_G, "deployCardScrollY") then deployCardScrollY = 0 end  -- 重置卡牌滚动
    ...
    autoSkillState.nextTime = 5.0  -- 开战后5秒自动释放首次武技
    ...
    -- === 计算SLG战斗动态缩放 & 精灵数量 ===
    local MAX_SPRITES_PER_SIDE = gameSettings.spriteLimit or 1000  -- ★ 精灵上限(每方, 设置可调500~5000)
    ...
    -- === 战旗回合制: 仅在明确指定战旗模式时启用 ===
    if gameState.useTacticsMode then
        local TacticsModule = require("systems.battle.tactics")
        gameState.battlePhase = "TACTICS"
        TacticsModule.Init()
        -- 记录初始兵力 (用于胜负判定和星级计算)
        gameState.initialPlayerUnits = #tacticState.playerGroups
        gameState.initialEnemyUnits  = #tacticState.enemyGroups
        print(string.format("=== 战旗模式启动 | 玩家兵团:%d 敌方兵团:%d ===",
            #tacticState.playerGroups, #tacticState.enemyGroups))
        return  -- 跳过后续所有预生成/排列逻辑
    end

    -- === [旧逻辑-已跳过] 预生成玩家单位 (按阵型排列, 冻结在己方区域) ===
...
function StartBattleFromDeploy()
    ...
    -- 记录初始兵力 (用于胜负判定和星级计算)
    gameState.initialPlayerUnits = #tbState.playerRegiments
    gameState.initialEnemyUnits = #tbState.enemyRegiments
    ...
    print(string.format("=== 回合制出征! | 玩家军团:%d 敌方军团:%d ===",
...
function SwitchDeployFormation(formId)
    ...
    deployCustomZones = nil       -- 切换阵型时清除手动交换
    deploySwapFirstTroop = nil
...
--- 交换两个兵种在当前阵型中的部署位置 (DEPLOY阶段)
--- @param troopA string 兵种key (如 "infantry")
--- @param troopB string 兵种key (如 "archer")
function SwapDeployTroopZones(troopA, troopB)
    if troopA == troopB then return end
    if not gameState or gameState.battlePhase ~= "DEPLOY" then return end

    -- 获取当前生效的区域配置
    local baseZones
    if deployCustomZones then
        baseZones = deployCustomZones
    else
        local formId = deploySelectedFormation or "fish_scale"
        local form = nil
        for _, f in ipairs(FORMATIONS) do if f.id == formId then form = f; break end end
        baseZones = (form and form.troopZones) or DEFAULT_TROOP_ZONES
    end

    -- 深拷贝当前区域到自定义区域
    deployCustomZones = {}
    for k, v in pairs(baseZones) do
        deployCustomZones[k] = {}
        for kk, vv in pairs(v) do
            if type(vv) == "table" then
                deployCustomZones[k][kk] = { table.unpack(vv) }
            else
                deployCustomZones[k][kk] = vv
            end
        end
    end

    -- 交换两个兵种的 xRange
    local zA = deployCustomZones[troopA]
    local zB = deployCustomZones[troopB]
    if zA and zB then
        zA.xRange, zB.xRange = zB.xRange, zA.xRange
    end
    ...
    local nameA = TROOP_NAMES[troopA] or troopA
    local nameB = TROOP_NAMES[troopB] or troopB
    AddFloatText(DESIGN_W / 2, DESIGN_H * 0.4, nameA .. " ⇄ " .. nameB, 1.0, { 120, 220, 255 }, 18)
    PlaySFX(AUDIO.sfx_click)
    print(string.format("[SwapDeployTroopZones] %s ⇄ %s", troopA, troopB))
end
...
function AggregateBaseStats()
    ...
    for _, slot in ipairs(PLAYER_SLOTS) do
        if slot.filled and slot.card then
            ...
            -- 六德兵符加成 (五维+暴击, 通过 stats5 间接影响战斗)
            local cardIdx = card.cardIdx
            if cardIdx then
                local sb = GetSealTotalBonus(cardIdx)
                -- 兵符加成叠加到武将五维上, 战斗时由五维系统自动转化为战斗属性
                if not card.stats5 then card.stats5 = { str = 50, int = 50, vit = 50, tec = 50, spd = 50 } end
                card.stats5._sealApplied = card.stats5._sealApplied or {}
                -- 先撤销上次叠加(防重复)
                local prev = card.stats5._sealApplied
                card.stats5.str = card.stats5.str - (prev.strAdd or 0) + sb.strAdd
                card.stats5.int = card.stats5.int - (prev.intAdd or 0) + sb.intAdd
                card.stats5.vit = card.stats5.vit - (prev.vitAdd or 0) + sb.vitAdd
                card.stats5.tec = card.stats5.tec - (prev.tecAdd or 0) + sb.tecAdd
                card.stats5.spd = card.stats5.spd - (prev.spdAdd or 0) + sb.spdAdd
                card.stats5._sealApplied = { strAdd = sb.strAdd, intAdd = sb.intAdd, vitAdd = sb.vitAdd, tecAdd = sb.tecAdd, spdAdd = sb.spdAdd }
                -- 缓存兵符加成供战斗使用(暴击率由 units.lua 读取)
