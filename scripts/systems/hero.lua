...
function GetPlayerTotalScore()
    ...
    for heroIdx, heroEquip in pairs(playerEquipment.equipped) do
        for slotIdx = 1, 7 do
            score = score + GetEquippedSlotScore(slotIdx, heroIdx)
        end
    end
...
function CalcHeroPowerScore(card)
    ...
    -- 六德兵符加成 (五维→战斗属性: str→ATK+0.3%, vit→HP+0.3%)
    if card.cardIdx then
        ...
        atk = math.floor(atk * (1 + sealStr * 0.003))
        hp  = math.floor(hp  * (1 + sealVit * 0.003))
    end
...
function CalcEquipPowerScore()
    ...
    for heroIdx, heroEquip in pairs(playerEquipment.equipped) do
        for slotIdx = 1, 7 do
            power = power + GetEquippedSlotScore(slotIdx, heroIdx)
        end
    end
    return power
end
