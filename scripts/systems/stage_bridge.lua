...
local function buildRankedCardFromHero(cardIdx, heroInfo)
    ...
    -- 六德兵符: 五维加成间接影响战力 (str→ATK+0.3%, vit→HP+0.3%, spd→速度+0.2%, tec→暴击+0.1%, int→技能+0.3%)
    local s5 = tempCard.stats5 or { str = 50, int = 50, vit = 50, tec = 50, spd = 50 }
    local sealStr = s5.str + (sealBonus.strAdd or 0)
    local sealVit = s5.vit + (sealBonus.vitAdd or 0)
    local finalAtk = math.floor(stats.atk * levelMul * (1 + sealStr * 0.003))
    local finalDef = math.floor(stats.def * levelMul)
    local finalHp = math.floor(stats.hp * levelMul * (1 + sealVit * 0.003))
