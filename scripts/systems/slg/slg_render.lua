...
local function DrawTopBar(W, H)
    ...
    -- 放弃征途按钮 (规则左侧)
    local abandonBtnX = rulesBtnX - saveBtnW - 4
    st.btn_abandon = M.DrawBtn(abandonBtnX, saveBtnY, saveBtnW, saveBtnH, "放弃", 160, 60, 50)
    -- 返回大厅按钮 (放弃左侧)
    local backBtnX = abandonBtnX - saveBtnW - 4
...
local function DrawBattleAnimMap(t)
    ...
    local dx_all, dy_all = tx - fx, ty - fy
    local dist = math.sqrt(dx_all * dx_all + dy_all * dy_all)
    local fc_atk = FC[data.fac] or FC.qun
    local cr, cg, cb = fc_atk.main[1], fc_atk.main[2], fc_atk.main[3]

    -- ---- march: 行军路线 + 部队旗帜 + 尘土飞扬 ----
    if phase == "march" then
        local dur = 1.5
        local p = math.min(1, at / dur)
        local dx, dy = dx_all, dy_all
        local ang = math.atan(dy, dx)  -- 行军方向角
        ...
        -- 外层宽描边 (半透明阵营色)
        nvgStrokeWidth(vg, 5)
        nvgStrokeColor(vg, nvgRGBA(cr, cg, cb, math.floor(60 * p)))
        nvgBeginPath(vg)
        nvgMoveTo(vg, fx, fy)
        nvgLineTo(vg, fx + dx * p, fy + dy * p)
        nvgStroke(vg)
        -- 内层窄描边 (高亮)
        nvgStrokeWidth(vg, 2)
        nvgStrokeColor(vg, nvgRGBA(255, 240, 200, math.floor(140 * p)))
        nvgBeginPath(vg)
        nvgMoveTo(vg, fx, fy)
        nvgLineTo(vg, fx + dx * p, fy + dy * p)
        nvgStroke(vg)
        -- 流动虚线动画 (沿路径移动的小点)
        local dotSpacing = 14
        local dots = math.max(1, math.floor(dist * p / dotSpacing))
        local flowOff = (at * 30) % dotSpacing  -- 流动偏移
        for i = 0, dots do
            local d = (i * dotSpacing + flowOff)
            if d <= dist * p then
                local frac = d / dist
                local dotX = fx + dx * frac
                local dotY = fy + dy * frac
                local dotA = math.floor(180 * (1 - frac / p * 0.3))
                nvgBeginPath(vg); nvgCircle(vg, dotX, dotY, 1.5)
                nvgFillColor(vg, nvgRGBA(255, 230, 160, dotA)); nvgFill(vg)
            end
        end
        nvgRestore(vg)
        ...
        if p > 0.05 and p < 0.95 then
            ...
            for i = 1, dustCount do
                local seed = (i * 7 + math.floor(at * 10)) * 13
                local life = ((seed % 100) / 100) * 0.6
                local phase2 = (at * 2 + i * 0.3) % 1.0
                local dOff = -8 - i * 4  -- 在部队后方
                local dsx = px + math.cos(ang + math.pi) * (8 + i * 5) + math.sin(ang) * ((seed % 20) - 10) * 0.3
                local dsy = py + math.sin(ang + math.pi) * (8 + i * 5) + math.cos(ang) * ((seed % 20) - 10) * 0.3
                local dAlpha = math.floor(80 * (1 - phase2))
                local dSize = 2 + phase2 * 4
                nvgBeginPath(vg); nvgCircle(vg, dsx, dsy - phase2 * 6, dSize)
                nvgFillColor(vg, nvgRGBA(180, 160, 120, dAlpha)); nvgFill(vg)
            end
        end

        -- (4) 部队主体 (带渐变光晕 + 旗帜)
        -- 外层大光晕
        local glow1 = nvgRadialGradient(vg, px, py, 4, 22,
            nvgRGBA(cr, cg, cb, 100), nvgRGBA(cr, cg, cb, 0))
        nvgBeginPath(vg); nvgCircle(vg, px, py, 22)
        nvgFillPaint(vg, glow1); nvgFill(vg)
        -- 内圈实心 (阵营色 + 金色边框)
        nvgBeginPath(vg); nvgCircle(vg, px, py, 10)
        nvgFillColor(vg, nvgRGBA(cr, cg, cb, 230)); nvgFill(vg)
        ...
        nvgText(vg, px, py, "⚔", nil)

        -- (5) 小旗帜 (飘动效果)
        local flagX2 = px - 6
        local flagY2 = py - 16
        local wave = math.sin(at * 8) * 2
        -- 旗杆
        nvgBeginPath(vg)
        nvgMoveTo(vg, flagX2, flagY2 + 8)
        nvgLineTo(vg, flagX2, flagY2 - 6)
        nvgStrokeColor(vg, nvgRGBA(180, 140, 80, 220))
        nvgStrokeWidth(vg, 1.5); nvgStroke(vg)
        -- 旗面 (带飘动)
        nvgBeginPath(vg)
        nvgMoveTo(vg, flagX2, flagY2 - 6)
        nvgLineTo(vg, flagX2 + 10 + wave, flagY2 - 4 + wave * 0.3)
        nvgLineTo(vg, flagX2 + 8 + wave * 0.5, flagY2)
        nvgLineTo(vg, flagX2, flagY2 - 1)
        nvgClosePath(vg)
        nvgFillColor(vg, nvgRGBA(cr, cg, cb, 230)); nvgFill(vg)

        -- (6) 兵力文字
        if data.troops then
            M.DrawTextOutlined(px, py - 24, FormatTroops(data.troops), 18, 240, NVG_ALIGN_CENTER + NVG_ALIGN_BOTTOM)
        end

        -- (7) 地面震动线 (部队行进产生的地表波纹)
        if p > 0.1 and p < 0.9 then
            for gi = 1, 3 do
                local gp = (at * 1.5 + gi * 0.3) % 1.0
                local gDist = 6 + gp * 12
                local gAlpha = math.floor(50 * (1 - gp))
                local gx1 = px - 15
                local gx2 = px + 15
                local gy = py + 8 + gDist
                nvgBeginPath(vg)
                nvgMoveTo(vg, gx1, gy)
                nvgBezierTo(vg, gx1 + 8, gy - 2 * (1 - gp), gx2 - 8, gy + 2 * (1 - gp), gx2, gy)
                nvgStrokeColor(vg, nvgRGBA(160, 140, 100, gAlpha))
                nvgStrokeWidth(vg, 1); nvgStroke(vg)
            end
        end

    -- ---- siege: 攻城 (火焰 + 飞石 + 冲击波 + 烟尘) ----
    elseif phase == "siege" then
        local dur = 0.6
        local p = math.min(1, at / dur)

        -- (1) 飞石轨迹 (3条抛物线从进攻方飞向城池)
        if p < 0.5 then
            local sp = p / 0.5  -- 飞石阶段进度 0~1
            for i = 1, 3 do
                local offX = (i - 2) * 15
                local offY = (i - 2) * 8
                local sx = fx + (tx - fx) * sp + offX
                local sy = fy + (ty - fy) * sp + offY - math.sin(sp * math.pi) * (30 + i * 10)
                -- 飞石 (深灰)
                nvgBeginPath(vg); nvgCircle(vg, sx, sy, 3 - sp)
                nvgFillColor(vg, nvgRGBA(100, 80, 60, math.floor(220 * (1 - sp)))); nvgFill(vg)
                -- 尾焰
                local tailLen = 8
                local tailX = sx - (tx - fx) / math.max(1, dist) * tailLen
                local tailY = sy - (ty - fy) / math.max(1, dist) * tailLen + 3
                nvgBeginPath(vg)
                nvgMoveTo(vg, sx, sy)
                nvgLineTo(vg, tailX, tailY)
                nvgStrokeColor(vg, nvgRGBA(255, 160, 40, math.floor(150 * (1 - sp))))
                nvgStrokeWidth(vg, 2 - sp); nvgStroke(vg)
            end
        end

        -- (2) 冲击波扩散 (从城池向外扩散4圈, p > 0.3 开始)
        if p > 0.3 then
            local wp = (p - 0.3) / 0.7
            for ring = 1, 4 do
                local rp = math.max(0, wp - (ring - 1) * 0.12)
                if rp > 0 then
                    local radius = 10 + 50 * rp
                    local alpha = math.floor(180 * (1 - rp) * (1 - rp))
                    local sw = math.max(0.5, 3.5 - rp * 3)
                    nvgBeginPath(vg); nvgCircle(vg, tx, ty, radius)
                    nvgStrokeColor(vg, nvgRGBA(255, 140 + ring * 20, 30, alpha))
        ...
        -- (3) 中心火焰爆裂 (径向渐变 + 多层)
        local fireP = math.max(0, p - 0.2) / 0.8
        if fireP > 0 then
            -- 外层火焰光晕 (橙红)
            local r1 = 6 + 30 * fireP
            local fa1 = math.floor(200 * (1 - fireP))
            local fg1 = nvgRadialGradient(vg, tx, ty, 2, r1,
                nvgRGBA(255, 200, 60, fa1), nvgRGBA(255, 80, 20, 0))
            nvgBeginPath(vg); nvgCircle(vg, tx, ty, r1)
            nvgFillPaint(vg, fg1); nvgFill(vg)
            -- 内层白色闪光
            local r2 = 4 + 10 * fireP
            local fa2 = math.floor(255 * math.max(0, 1 - fireP * 2))
            nvgBeginPath(vg); nvgCircle(vg, tx, ty, r2)
            nvgFillColor(vg, nvgRGBA(255, 255, 220, fa2)); nvgFill(vg)
        end
        ...
        if p > 0.4 then
            local dp = (p - 0.4) / 0.6
            for i = 1, 6 do
                local a2 = (i / 6) * math.pi * 2 + 0.5
                local debrisDist = 10 + 40 * dp
                local dbx = tx + math.cos(a2) * debrisDist
                local dby = ty + math.sin(a2) * debrisDist - dp * 15
                local dba = math.floor(180 * (1 - dp))
                local dbs = 2 - dp * 1.5
                if dbs > 0.3 then
                    nvgBeginPath(vg); nvgCircle(vg, dbx, dby, dbs)
                    nvgFillColor(vg, nvgRGBA(160, 120, 80, dba)); nvgFill(vg)
                end
            end
        end
        ...
        -- (5) 烟尘上升 (p > 0.5)
        if p > 0.5 then
            local smokep = (p - 0.5) / 0.5
            for i = 1, 4 do
                local sOff = (i - 2.5) * 8
                local smokeY = ty - 10 - smokep * 25 - i * 3
                local smokeA = math.floor(80 * (1 - smokep))
                local smokeR = 4 + smokep * 8 + i
                nvgBeginPath(vg); nvgCircle(vg, tx + sOff, smokeY, smokeR)
                nvgFillColor(vg, nvgRGBA(100, 90, 80, smokeA)); nvgFill(vg)
            end
        end

        -- (6) 火花飞溅 (攻城撞击产生的星形火花, p > 0.2)
        if p > 0.2 then
            local fkp = (p - 0.2) / 0.8
            for i = 1, 8 do
                local ang2 = (i / 8) * math.pi * 2 + at * 3
                local fkDist = 5 + 35 * fkp * (0.8 + (i % 3) * 0.1)
                local fkx = tx + math.cos(ang2) * fkDist
                local fky = ty + math.sin(ang2) * fkDist - fkp * 8
                local fkAlpha = math.floor(200 * (1 - fkp))
                local fkSz = 1.5 - fkp
                if fkSz > 0.2 then
                    -- 菱形火花
                    nvgBeginPath(vg)
                    nvgMoveTo(vg, fkx, fky - fkSz * 2)
                    nvgLineTo(vg, fkx + fkSz, fky)
                    nvgLineTo(vg, fkx, fky + fkSz * 2)
                    nvgLineTo(vg, fkx - fkSz, fky)
                    nvgClosePath(vg)
                    nvgFillColor(vg, nvgRGBA(255, 200 + (i * 20) % 55, 40, fkAlpha)); nvgFill(vg)
                end
            end
        end

    -- ---- capture: 城池变色 + 旗帜升起 + 光柱 + 阵营印记 ----
    elseif phase == "capture" then
        ...
        -- (1) 光柱效果 (从地面升向天空)
        local beamH = 80 * p
        local beamW = 6 + 4 * math.sin(at * 6)
        local beamA = math.floor(160 * (1 - p * 0.4))
        local beamGrad = nvgLinearGradient(vg, tx, ty, tx, ty - beamH,
            nvgRGBA(cr, cg, cb, beamA), nvgRGBA(cr, cg, cb, 0))
        nvgBeginPath(vg)
        nvgMoveTo(vg, tx - beamW / 2, ty)
        nvgLineTo(vg, tx - beamW * 0.3, ty - beamH)
        nvgLineTo(vg, tx + beamW * 0.3, ty - beamH)
        nvgLineTo(vg, tx + beamW / 2, ty)
        nvgClosePath(vg)
        nvgFillPaint(vg, beamGrad); nvgFill(vg)

        -- (2) 底部扩散光环 (脉冲)
        local pulse = 0.5 + 0.5 * math.sin(p * math.pi * 4)
        local glowR = 18 + 35 * p
        local ringGrad = nvgRadialGradient(vg, tx, ty, 4, glowR,
            nvgRGBA(cr, cg, cb, math.floor(140 * pulse)), nvgRGBA(cr, cg, cb, 0))
        nvgBeginPath(vg); nvgCircle(vg, tx, ty, glowR)
        nvgFillPaint(vg, ringGrad); nvgFill(vg)
        -- 光环描边
        nvgBeginPath(vg); nvgCircle(vg, tx, ty, glowR)
        nvgStrokeColor(vg, nvgRGBA(cr, cg, cb, math.floor(200 * pulse * (1 - p * 0.3))))
        nvgStrokeWidth(vg, 2); nvgStroke(vg)

        -- (3) 旗帜升起 (带旗杆 + 飘动旗面)
        local flagRise = math.min(1, p * 1.5)  -- 旗帜比其他元素更快升起
        local flagBaseX = tx + CITY_ICON_SIZE * 0.32
        local flagTopY = ty - CITY_ICON_SIZE * 0.38 - 16
        local flagBotY = ty - CITY_ICON_SIZE * 0.15
        local curFlagTop = flagBotY + (flagTopY - flagBotY) * flagRise
        local flagAlpha = math.floor(255 * math.min(1, p * 2))
        -- 旗杆
        nvgBeginPath(vg)
        nvgMoveTo(vg, flagBaseX, flagBotY)
        nvgLineTo(vg, flagBaseX, curFlagTop)
        nvgStrokeColor(vg, nvgRGBA(200, 170, 100, flagAlpha))
        nvgStrokeWidth(vg, 2); nvgStroke(vg)
        -- 旗杆顶端金球
        nvgBeginPath(vg); nvgCircle(vg, flagBaseX, curFlagTop, 2)
        nvgFillColor(vg, nvgRGBA(255, 220, 100, flagAlpha)); nvgFill(vg)
        -- 旗面 (带飘动)
        if flagRise > 0.3 then
            local fw = math.sin(at * 10) * 2.5
            nvgBeginPath(vg)
            nvgMoveTo(vg, flagBaseX, curFlagTop)
            nvgLineTo(vg, flagBaseX + 14 + fw, curFlagTop + 3 + fw * 0.3)
            nvgLineTo(vg, flagBaseX + 12 + fw * 0.5, curFlagTop + 9)
            nvgLineTo(vg, flagBaseX, curFlagTop + 7)
            nvgClosePath(vg)
            nvgFillColor(vg, nvgRGBA(cr, cg, cb, flagAlpha)); nvgFill(vg)
            -- 旗面高光条纹
            nvgBeginPath(vg)
            nvgMoveTo(vg, flagBaseX + 2, curFlagTop + 2)
            nvgLineTo(vg, flagBaseX + 10 + fw * 0.6, curFlagTop + 4)
            nvgStrokeColor(vg, nvgRGBA(255, 255, 255, math.floor(flagAlpha * 0.3)))
            nvgStrokeWidth(vg, 1); nvgStroke(vg)
        end
        ...
        for i = 1, sparkCount do
            local sp2 = (p + i * 0.15) % 1.0
            local sx = tx + math.sin(i * 2.1 + at * 3) * (10 + i * 3)
            local sy = ty - sp2 * 50 - i * 2
            local sa = math.floor(180 * (1 - sp2))
            local ss = 1.5 + (1 - sp2) * 2
            nvgBeginPath(vg); nvgCircle(vg, sx, sy, ss)
            nvgFillColor(vg, nvgRGBA(cr, cg, cb, sa)); nvgFill(vg)
        end
    ...
    elseif phase == "cheer" then
        ...
        -- (1) 烟花爆发 (3组, 依次引爆)
        for burst = 1, 3 do
            local bStart = (burst - 1) * 0.15
            local bp = math.max(0, (p - bStart) / (1 - bStart))
            if bp > 0 then
                -- 每组烟花中心偏移
                local bcx = tx + (burst - 2) * 20
                local bcy = ty - 15 - burst * 8
                local sparkN = 10
                for i = 1, sparkN do
                    local a3 = (i / sparkN) * math.pi * 2 + burst * 1.2
                    local sDist = (8 + 30 * bp) * (0.7 + (i % 3) * 0.15)
                    local sx = bcx + math.cos(a3) * sDist
                    local sy = bcy + math.sin(a3) * sDist + bp * bp * 8  -- 重力下坠
                    local sAlpha = math.floor(220 * (1 - bp * 0.7))
                    local sSize = (2.5 - bp * 1.5)
                    if sSize > 0.3 then
                        nvgBeginPath(vg); nvgCircle(vg, sx, sy, sSize)
                        -- 三组颜色: 金/阵营/白
                        if burst == 1 then
                            nvgFillColor(vg, nvgRGBA(255, 220, 80, sAlpha))
                        elseif burst == 2 then
                            nvgFillColor(vg, nvgRGBA(cr, cg, cb, sAlpha))
                        else
                            nvgFillColor(vg, nvgRGBA(255, 240, 220, sAlpha))
                        end
                        nvgFill(vg)
                    end
                end
                -- 烟花中心闪光
                if bp < 0.3 then
                    local cfa = math.floor(255 * (1 - bp / 0.3))
                    nvgBeginPath(vg); nvgCircle(vg, bcx, bcy, 4 - bp * 8)
                    nvgFillColor(vg, nvgRGBA(255, 255, 240, cfa)); nvgFill(vg)
                end
            end
        end
        ...
        local goldN = 12
        for i = 1, goldN do
            local gx = tx + math.sin(i * 1.7 + at * 2) * 35
            local fallP = (p + i * 0.07) % 1.0
            local gy = ty - 40 + fallP * 80
            local ga = math.floor(160 * (1 - fallP * 0.8))
            -- 小矩形碎片 (旋转)
            nvgSave(vg)
            nvgTranslate(vg, gx, gy)
            nvgRotate(vg, at * 5 + i * 0.8)
            nvgBeginPath(vg); nvgRect(vg, -1.5, -1, 3, 2)
            nvgFillColor(vg, nvgRGBA(255, 210 + (i % 3) * 15, 60, ga)); nvgFill(vg)
            nvgRestore(vg)
        end

        -- (3) 中心星爆光环 (渐弱)
        local starA = math.floor(180 * (1 - p))
        local starR = 10 + 20 * p
        local sGrad = nvgRadialGradient(vg, tx, ty, 2, starR,
            nvgRGBA(255, 240, 180, starA), nvgRGBA(255, 200, 80, 0))
        nvgBeginPath(vg); nvgCircle(vg, tx, ty, starR)
        nvgFillPaint(vg, sGrad); nvgFill(vg)

        -- (4) 彩带飘洒 (S形下落)
        local ribbonN = 6
        for i = 1, ribbonN do
            local rx = tx + (i - 3.5) * 12
            local rFall = (p + i * 0.1) % 1.0
            local ry1 = ty - 30 + rFall * 60
            local ry2 = ry1 + 4
            local rWave = math.sin(at * 6 + i * 1.5) * 4
            local ra = math.floor(180 * (1 - rFall * 0.5))
            nvgBeginPath(vg)
            nvgMoveTo(vg, rx + rWave, ry1)
            nvgBezierTo(vg, rx + rWave + 3, ry1 + 1.5, rx + rWave - 3, ry2 - 1.5, rx + rWave, ry2)
            nvgStrokeColor(vg, nvgRGBA(cr + (i * 30) % 80, cg + (i * 20) % 60, cb + (i * 40) % 100, ra))
            nvgStrokeWidth(vg, 2); nvgStroke(vg)
        end

        -- (5) 底部辉光脉冲 (胜利光芒从城池底部扩散)
        local glowP = 0.5 + 0.5 * math.sin(at * 8)
        local btmGlowR = 25 + 15 * p
        local btmGrad = nvgRadialGradient(vg, tx, ty + 5, 3, btmGlowR,
            nvgRGBA(255, 230, 120, math.floor(100 * glowP * (1 - p * 0.5))),
            nvgRGBA(255, 200, 60, 0))
        nvgBeginPath(vg); nvgEllipse(vg, tx, ty + 5, btmGlowR, btmGlowR * 0.5)
        nvgFillPaint(vg, btmGrad); nvgFill(vg)
    end
...
local function DrawHeroPopup(W, H, t)
    ...
    DrawInventoryCard(cardAreaX, cardAreaY, cardW2, cardH2, card, cons, false, false, true)
...
function M.DrawWorldMapScreen(drawPanelFn)
    ...
    -- === 7.8 放弃征途确认弹窗 ===
    if worldMapState.abandonConfirm then
        -- 全屏遮罩
        nvgBeginPath(vg); nvgRect(vg, 0, 0, W, H)
        nvgFillColor(vg, nvgRGBA(0, 0, 0, 180)); nvgFill(vg)
        -- 弹窗
        local apw, aph = 280, 140
        local apx, apy = (W - apw) / 2, (H - aph) / 2
        nvgBeginPath(vg); nvgRoundedRect(vg, apx, apy, apw, aph, 10)
        nvgFillColor(vg, nvgRGBA(30, 25, 18, 245)); nvgFill(vg)
        nvgStrokeColor(vg, nvgRGBA(200, 160, 80, 150)); nvgStrokeWidth(vg, 1.5); nvgStroke(vg)
        -- 标题
        nvgFontFaceId(vg, GetMainFont())
        nvgTextAlign(vg, NVG_ALIGN_CENTER + NVG_ALIGN_TOP)
        nvgFontSize(vg, 20); nvgFillColor(vg, nvgRGBA(255, 220, 120, 255))
        nvgText(vg, W / 2, apy + 16, "放弃征途", nil)
        -- 提示文字
        nvgFontSize(vg, 14); nvgFillColor(vg, nvgRGBA(220, 210, 190, 210))
        nvgText(vg, W / 2, apy + 48, "确定放弃当前征途？", nil)
        nvgText(vg, W / 2, apy + 66, "进度将清除，可重新选择剧本", nil)
        -- 按钮
        local btnW2, btnH2 = 100, 32
        local btnGap = 20
        local btnY2 = apy + aph - 46
        local bx1 = W / 2 - btnW2 - btnGap / 2
        local bx2 = W / 2 + btnGap / 2
        worldMapState.btn_abandonCancel = M.DrawBtn(bx1, btnY2, btnW2, btnH2, "取消", 80, 70, 60)
        worldMapState.btn_abandonOk     = M.DrawBtn(bx2, btnY2, btnW2, btnH2, "确定", 180, 60, 50)
    end
