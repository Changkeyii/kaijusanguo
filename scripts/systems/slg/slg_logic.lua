...
function M.LearnSkill(studentIdx, teacherIdx)
    ...
    if math.random() < successRate then
        ...
        -- 自动装备: 如果该武技已解锁且武技槽有空位, 自动装备
        if SKILL_DEFS[teacherTech] and SKILL_DEFS[teacherTech].unlocked then
            local already = false
            for _, si in ipairs(playerEquippedSkills) do
                if si == teacherTech then already = true; break end
            end
            if not already and #playerEquippedSkills < 2 then
                playerEquippedSkills[#playerEquippedSkills + 1] = teacherTech
                if rawget(_G, "SaveGameProgress") then SaveGameProgress() end
                print("=== 自动装备武技: " .. techName .. " (槽位" .. #playerEquippedSkills .. ") ===")
            end
        end
...
function M.StartAttack(fromId, toId)
    ...
    -- 敌方缩放: 基于城池等级和武将数(守将用真实HERO_CARDS, 不再需要补偿)
    local defStrength = toData.level * 1200 + #toData.heroes * 800
    local scale = 0.8 + defStrength / 8000
            ...
            for i, hIdx in ipairs(defHeroes) do
                ...
                if card then
                    ...
                    slot.card = {
                        ...
                        atk = math.floor(card.atk * eScale),
                        def = math.floor(card.def * eScale),
                        hp  = math.floor(card.hp  * eScale),
                        skill = card.skill, skillData = card.skillData, singleImg = card.singleImg,
                        row = card.row, col = card.col, type = card.type,
            ...
            if filled < 2 and rawget(_G, "ENEMY_CARDS") then
                ...
                for fi = filled + 1, math.min(3, #ENEMY_SLOTS) do
                    ...
                    if ec and not ec.isBoss then
                        ...
                        -- 泛用兵卒用较低缩放, 避免比守将还强
                        local fillScale = eScale * 0.5
                        card.atk = math.floor(card.atk * fillScale)
                        card.def = math.floor(card.def * fillScale)
                        card.hp  = math.floor(card.hp  * fillScale)
