...
function M.DrawMapPanel(rpX, rpY, rpW, rpH, t)
    ...
    if st.selectedCity then
        ...
        -- 武将小图列表 (固定区域 + 滚动)
        infoY = infoY + lineH
        -- 计算底部按钮区域高度 (我方6按钮3行, 敌方2行文字)
        local btnAreaH = isPlayer and (30 * 3 + 5 * 2 + 16) or 50
        ...
        if st.scoutResult and st.scoutResult.cityId == st.selectedCity and not isPlayer then
            ...
            scoutH = 16
            if #sr.heroes > 0 then
                nvgText(vg, rpX + pad, infoY + 16, "将:" .. table.concat(sr.heroes, " "), nil)
                scoutH = 32
            end
        end
        ...
        if isPlayer then
            ...
            local btnH = 30
            local btnGap = 5
            local btnStartY = rpY + rpH - (btnH * 3 + btnGap * 2) - 8
...
function M.DrawDiplomacyPanel(rpX, rpY, rpW, rpH, t)
    ...
    for i, fac in ipairs(factions) do
        ...
        -- 关系条
        local barX = rpX + pad + 35
        local barW2, barH2 = 80, 5
        ...
        nvgFontSize(vg, 11)
        nvgFillColor(vg, nvgRGBA(200, 190, 170, 200))
        nvgText(vg, barX + barW2 + 4, fy + 2, tostring(d.relation), nil)
        if d.treaty then
            nvgFillColor(vg, nvgRGBA(80, 200, 100, 230))
            nvgText(vg, barX + barW2 + 26, fy + 2, "[和约]", nil)
        end

        -- 城数
        nvgFillColor(vg, nvgRGBA(180, 170, 150, 170))
        nvgText(vg, rpX + rpW - pad - 30, fy + 2, "城:" .. (fs[fac] and fs[fac].cities or 0), nil)

        -- 按钮 (第二行)
        local btnW2, btnH2 = 75, 24
        local btnY2 = fy + 20
        st["btn_gift_" .. fac] = DrawBtn(rpX + pad, btnY2, btnW2, btnH2, "送礼200金",
            80, 100, 60, st.gold >= 200 and 200 or 100)
        if not d.treaty then
            st["btn_treaty_" .. fac] = DrawBtn(rpX + pad + btnW2 + 6, btnY2, btnW2, btnH2, "缔约500金",
...
function M.DrawStratagemPanel(rpX, rpY, rpW, rpH, t)
    ...
    for i, strat in ipairs(STRATAGEMS) do
        ...
        nvgText(vg, rpX + pad + 70, by + btnH3 / 2 - 6, strat.desc, nil)
    ...
    if st.turnReport then
        ...
        for i, item in ipairs(st.turnReport) do
            ...
            if lineY >= contentTop - lineH and lineY < contentBot then
                ...
                nvgText(vg, rpX + pad, lineY, icon .. " " .. text, nil)
            end
        end
        ...
        if #st.turnReport > maxVisible then
            ...
            nvgText(vg, rpX + rpW - pad, contentBot - 5, "↕滑动", nil)
        end
    end
...
function M.DrawSurrenderPanel(rpX, rpY, rpW, rpH, t)
    ...
    if #heroes == 0 then
        ...
        for i, hIdx in ipairs(heroes) do
            ...
            local oy = startY + (i - 1) * rowH
            if oy + rowH > rpY + rpH - 42 then break end  -- 留底部按钮空间
            ...
            nvgFillColor(vg, nvgRGBA(60, 40, 20, alreadyDone and 40 or 70)); nvgFill(vg)
            nvgStrokeColor(vg, nvgRGBA(200, 170, 100, alreadyDone and 40 or 80))
            nvgStrokeWidth(vg, 1); nvgStroke(vg)
            ...
            nvgText(vg, rpX + pad + 8, oy + 6, card.name, nil)
            ...
            nvgText(vg, rpX + pad + 8, oy + 24, facName .. " " .. troopName, nil)

            -- 属性
            nvgText(vg, rpX + pad + 8, oy + 38, "攻:" .. card.atk .. " 防:" .. card.def .. " 血:" .. card.hp, nil)
            ...
            if alreadyDone then
                ...
                if result == true then
                    ...
                    nvgText(vg, rpX + rpW - pad - 6, oy + rowH / 2 - 2, "已拒绝", nil)
                end
            else
                local bx = rpX + rpW - pad - btnW2
                local by1 = oy + 4
                local by2 = oy + 4 + btnH2 + 4
                st["btn_surrender_" .. i] = DrawBtn(bx, by1, btnW2, btnH2, "招降", 140, 100, 40)
                st["btn_release_" .. i]   = DrawBtn(bx, by2, btnW2, btnH2, "释放", 80, 70, 60)
            end
            ::continue_surrender::
        end
    end
