...
-- ============================================================================
-- 多人模式下，数据修改必须经过服务器验证
-- 客户端只负责：1. 本地缓存（防崩溃）2. 通过 RPC 发送到服务端
local function _isServerAuthoritative()
    -- cl_state 由 Client.lua 在收到 STATE_SYNC 后设置
    return rawget(_G, "cl_state") ~= nil
        and rawget(_G, "netState") ~= nil
        and netState.connected
end

--- 获取 Client 模块（延迟 require 避免循环依赖）
local _clientModule = nil
local function _getClient()
    if not _clientModule then
        local ok, m = pcall(require, "network.Client")
        if ok then _clientModule = m end
    end
...
function CloudManager.SaveAll(callback, forceBypass)
    ...
    -- 2. 保存本地JSON (无论哪种模式都保留本地兜底)
    CloudManager._saveLocalJSON(allData)

    -- ================================================================
    -- 服务端权威模式: 通过 RPC 发送到服务端
    -- ================================================================
    if _isServerAuthoritative() then
        local Client = _getClient()
        if not Client then
            if callback then callback(true, "本地保存成功(Client不可用)") end
            return
        end
        ...
        Client.SaveAllDomains(allData, function(ok, code, data, msg)
            if ok then
                print("[CloudManager] 服务端全量保存成功")
                S.retryCount = 0
                S.lastSyncTime = os.time()
                if callback then callback(true, "服务端保存成功") end
            else
                print("[CloudManager] 服务端保存失败: " .. tostring(msg))
    ...
    -- ================================================================
    -- 多人模式断连保护: netState 存在说明是多人模式，禁止 fallback clientCloud
    -- ================================================================
    if rawget(_G, "netState") then
        print("[CloudManager] 多人模式下服务器不可达, 仅本地保存(排队等待重连)")
        S.retryCount = S.retryCount + 1
        if S.retryCount <= 3 then
            S.retryTimer = 30
            S.retryData = allData
        end
        if callback then callback(false, "服务器不可达, 仅本地保存") end
        return
    end

    -- ================================================================
    -- 单机模式: clientCloud BatchSet（保留原有逻辑）
    -- ================================================================
...
function CloudManager.SaveDomain(domain, callback)
    ...
    if _isServerAuthoritative() then
        local Client = _getClient()
        if Client then
            Client.SaveDomain(domain, data, function(ok, code, respData, msg)
                if ok then
                    print("[CloudManager] domain " .. domain .. " 服务端保存成功")
                    if callback then callback(true) end
                else
                    print("[CloudManager] domain " .. domain .. " 服务端保存失败: " .. tostring(msg))
    ...
    -- 多人模式断连保护: 禁止 fallback clientCloud
    if rawget(_G, "netState") then
        print("[CloudManager] 多人模式下服务器不可达, domain " .. domain .. " 仅本地保存")
        if callback then callback(false) end
        return
    end
...
---@param callback? fun(source: string) "local" | "cloud" | "server" | "none"
function CloudManager.LoadAll(callback)
    -- ================================================================
    -- 服务端权威模式: 数据由 STATE_SYNC 推送，不走 clientCloud
    -- ================================================================
    if _isServerAuthoritative() then
        CloudManager._loadAllServerAuthoritative(callback)
        return
    end

    -- ================================================================
    -- 单机/旧模式: 本地 + clientCloud 对比（保留原有逻辑）
    -- ================================================================
    CloudManager._loadAllLegacy(callback)
end

--- 服务端权威模式的加载逻辑
---@param callback? fun(source: string)
function CloudManager._loadAllServerAuthoritative(callback)
    print("[CloudManager] 服务端权威模式: 从 cl_state 加载数据")

    -- 1. 加载本地 JSON 作为兜底（防止 STATE_SYNC 尚未到达的极端情况）
    local localData = CloudManager._loadLocalJSON()

    -- 2. 检查 cl_state 是否已由 STATE_SYNC 填充
    local cs = rawget(_G, "cl_state")
    if cs and cs.domains and next(cs.domains) then
        -- 服务端数据已到达，应用到游戏全局变量
        CloudManager._applyMultiDomain({ domains = cs.domains })
        CloudManager._sanitizeResources()

        -- 应用货币数据到 playerInfo
        if cs.money then
            if rawget(_G, "playerInfo") then
                if cs.money.jade then playerInfo.jade = cs.money.jade end
                if cs.money.lingshi then playerInfo.lingshi = cs.money.lingshi end
            end
        end
        ...
        print("[CloudManager] 服务端数据已应用, version=" .. tostring(cs.version))

        -- 3. 检查迁移状态
        if not cs.migrated and localData then
            CloudManager._tryMigrateToServer(localData)
        end
    ...
    -- 3. cl_state 尚未填充（极端情况：LoadAll 在 STATE_SYNC 之前被调用）
    -- 先用本地数据兜底，等待 STATE_SYNC 到达后会通过 OnFullStateSync 覆盖
    if localData then
        if localData._multiDomain then
            CloudManager._applyMultiDomain(localData)
        elseif rawget(_G, "ApplySaveData") then
            ApplySaveData(localData)
        end
        print("[CloudManager] cl_state 未就绪, 使用本地存档兜底")

        -- 注册一次性回调：STATE_SYNC 到达后重新应用
        if not CloudManager._pendingResync then
            CloudManager._pendingResync = true
            local origCb = rawget(_G, "OnFullStateSync")
            rawset(_G, "OnFullStateSync", function(newState)
                CloudManager._pendingResync = false
                -- 应用服务端数据覆盖本地兜底
                if newState and newState.domains then
                    CloudManager._applyMultiDomain({ domains = newState.domains })
                    CloudManager._sanitizeResources()
                    if newState.money and rawget(_G, "playerInfo") then
                        if newState.money.jade then playerInfo.jade = newState.money.jade end
                        if newState.money.lingshi then playerInfo.lingshi = newState.money.lingshi end
                    end
                    CloudManager._saveLocalJSON(nil)
                    print("[CloudManager] STATE_SYNC 到达, 服务端数据已覆盖本地兜底")

                    -- 检查迁移
                    if not newState.migrated and localData then
                        CloudManager._tryMigrateToServer(localData)
                    end
                end
                -- 恢复原回调
                if origCb then origCb(newState) end
            end)
        end

        if callback then callback("local") end
    else
        print("[CloudManager] cl_state 未就绪且本地无存档, 等待 STATE_SYNC")
        if callback then callback("none") end
    end
end

--- 尝试将旧 clientCloud 数据迁移到服务端
---@param localData table 本地存档数据
function CloudManager._tryMigrateToServer(localData)
    local Client = _getClient()
    if not Client then return end

    print("[CloudManager] 检测到未迁移, 准备迁移旧数据到服务端...")

    -- 收集所有 domain 的当前数据
    local allDomains = {}
    for name, _ in pairs(DOMAINS) do
        allDomains[name] = CloudManager.CollectDomainData(name)
    end

    -- 收集货币数据
    local money = {
        jade = (rawget(_G, "playerInfo") and playerInfo.jade) or 0,
        lingshi = (rawget(_G, "playerInfo") and playerInfo.lingshi) or 0,
    }

    Client.MigrateLegacy(allDomains, money, function(ok, code, data, msg)
        if ok then
            print("[CloudManager] 数据迁移成功!")
        else
            print("[CloudManager] 数据迁移失败: " .. tostring(msg) .. " (将在下次登录重试)")
        end
    end)
end

--- 单机/旧模式的加载逻辑（保留原有 clientCloud 逻辑）
---@param callback? fun(source: string)
function CloudManager._loadAllLegacy(callback)
    -- 1. 加载本地
    local localData = CloudManager._loadLocalJSON()
